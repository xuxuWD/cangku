    ...
    <Space direction="vertical" size={tokens.spacing.md} style={{ width: '100%' }}>
      ...
      {connected ? (
        <Alert
          ...
          description="列表与「启用 / 停用」走真实后端接口（读档已对四档业务角色放开并按「归属自己 ∪ 被共享」过滤；「启用 / 停用」仍仅超级管理员）。指标由真实列表派生（后端无聚合接口）；后端 status 无「草稿」枚举，故草稿卡按「未验证」；运行与成功率口径后端未提供，一律按「未验证 / 样本不足」呈现。后端不支持按创建者 / 名称筛选，这两项已禁用（透传会被忽略，得到未筛选的假结果）。"
        />
      ) : (
        <Alert
          type="warning"
