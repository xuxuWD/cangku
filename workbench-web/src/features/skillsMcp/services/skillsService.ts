...
/** 员工候选为空时的如实说明（**第 12 轮 ③A：不能再手动录入** ⇒ 给出纳管去处，不留空白下拉）。 */
export const AGENT_CANDIDATE_EMPTY_NOTE =
  '数字员工目录里还没有「启用」状态的员工；请先到「数字员工管理」纳管一个数字员工，之后它就会出现在这里。'

/** 员工键来源说明（第 12 轮 ③A：服务端要求已纳管且启用 ⇒ 界面只给目录候选，不提供手动录入）。 */
export const AGENT_DIRECTORY_ONLY_NOTE =
  '员工键只能从「数字员工管理」目录中选择：服务端要求该标识已纳管且启用，目录之外的标识会被拒绝（409）。'
