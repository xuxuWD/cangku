"""执行回调边车进程配置（规格 §3.5 P1 第 3 条 ②④ / §8 U20 方案 b-1 / §8 U21 裁决路线）。
...
`FORWARD_URL` 或 `SHARED_SECRET` 留空即**拒绝启动**（fail-closed）。
"""
...
class ExecCallbackConfig:
    ...
    - `shared_secret` ＝ **「边车 → 工作台」的预共享密钥**（对称；工作台侧同名配置项比对）；
      留空 ⇒ 拒绝启动（fail-closed；**不得烤进镜像**，只能由部署密钥系统外置注入）。
    - `listen_host` / `listen_port` ＝ 边车**唯一**监听面；对外可达性由「边车**双宿**
      `workbench-exec-internal` + 回调网」这一网络成员事实限定（**不得**加入工作台默认网络）。
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not (self.shared_secret or "").strip():
            raise ExecCallbackConfigError(
                "未配置「边车→工作台」预共享密钥（WORKBENCH_EXEC_CALLBACK_SHARED_SECRET），拒绝启动边车"
            )
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> "ExecCallbackConfig":
        ...
        return cls(
            ...
            shared_secret=_text(source, "SHARED_SECRET"),
