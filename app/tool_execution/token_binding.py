...
⚠️ **调用方（2026-09-14 更新）**：②④ 强制校验现由**工作台**的执行回调接收端点触发
（`app/tool_execution/callback_guard.py` 的 `WorkbenchCallbackGuard`，挂在
`POST /api/v1/internal/exec-callback`）。§8 U21 裁决「候选②」后，边车退化为**无状态纯转发**、
不再持有本模块；判定所需的权威状态（自持绑定 + 当前执行登记表）都在**工作台进程内**。
**「②④ 已强制」以该端点的端到端取证为准**（§5 用例 35：`tests/test_exec_callback.py`）。
"""
