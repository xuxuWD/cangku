...
| 测试用例数 | **1037**（后端）+ 前端 `admin-web` 18、`companion-pwa` 24、`desktop` 13（Node `node:test`） |
...
| 提交门禁 CI（push / PR 强制跑全量测试与编译） | ✅ | ✅ | ⬜ | `.github/workflows/ci.yml`（后端 pytest+compileall、两个前端 vitest+build、桌面 node --test）、`tests/test_ci_assets.py` 静态契约守护；**工作流实际执行未在本机复现，属未验收项** |
| 迁移回滚演练 | ❌ | ❌ | ⬜ | **未做** |
