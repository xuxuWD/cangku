...
>
> **生成日期**：2026-09-10；**2026-09-11 同步三态**（第一次：容器化、弱口令策略、TOTP 二次验证、攻击面报告；第二次：数据分级闸门、协同动态表现层、运行指标采集、编排优化提案、GEO 阻塞更正；第三次：待办聚合接口、PWA 伴侣端、Electron 桌面端）
> **基线**：分支 `feature/planning-closure`，全量 **662 项测试通过**，`compileall` 退出码 0
>
> **证据口径（重要）**：本清单以 `docs/delivery-gates.md` 的勾选状态、`tests/` 目录的 80 个测试文件、`README.md` 的能力声明为准。标 ✅ 表示仓库内存在实现且门禁已勾选，**不等于我逐项重新验收过**；需要真实环境证据的项一律标 ⬜。
...
| 测试文件数 | **80**（`tests/`，不含 `conftest.py`） |
| 测试用例数 | **662**（后端）+ 前端 `admin-web` 18、`companion-pwa` 24、`desktop` 13（Node `node:test`） |

> **一句话结论**：服务端能力基本齐全，**剩余卡点集中在「真实环境验收」「外部系统契约（GEO）」与「桌面端打包签名/PWA 真机」三处**，而不是功能缺口。
...
9. **合并本批次分支 `feature/planning-closure` 回 `main`**（本轮成果：后端 662 项 + `admin-web` 18 + `companion-pwa` 24 + `desktop` 13 全绿）。
10. **拿到外部资源后**：跑 `py scripts/staging_preflight.py` → 跨租户测试 → 并发压测 → 沙箱验证 → 真实联调。
11. **桌面端发布链**：固定依赖版本 → 代码签名与时间戳 → 安装包构建 → 干净 Windows 机器验收（阻塞项 7）。
12. **PWA 真机验收**：真机安装性验证；若要真推送再引入 HTTPS + VAPID（阻塞项 8）。
13. **GEO 适配器**：待 GEO 侧提供契约后再启动（见阻塞项 6），不做占位实现。
