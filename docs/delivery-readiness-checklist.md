...
>
> **生成日期**：2026-09-10
> **基线**：分支 `feature/audit-throttle`，全量 **439 项测试通过**，`compileall` 退出码 0
>
> **证据口径（重要）**：本清单以 `docs/delivery-gates.md` 的勾选状态、`tests/` 目录的 55 个测试文件、`README.md` 的能力声明为准。标 ✅ 表示仓库内存在实现且门禁已勾选，**不等于我逐项重新验收过**；需要真实环境证据的项一律标 ⬜。
...
| 维度 | 数量 |
| --- | --- |
| 门禁总项数 | **53**（`delivery-gates.md`） |
| 已勾选（实现） | **38** |
| 未勾选 | **15** |
| 达到**真实环境验收** | **0**（部分是本地真实验证，见「本地已验证」一行） |
| 本地已验证（真实 PostgreSQL 迁移与备份恢复演练、内容工作台内部闭环回归） | **2** |
| 测试文件数 | **55**（`tests/`，不含 `conftest.py`） |
| 测试用例数 | **439** |

> **一句话结论**：服务端能力基本齐全，**卡点全部集中在「真实环境验收」与「客户端」两端**，而不是功能缺口。
...
| 基础依赖、环境示例、Docker Compose | ✅ | — | ⬜ | `.env.example`、`docker-compose.yml`（**仅基础设施**） |
...
| 设备绑定、生产密钥轮换、真实统一登录验收 | ❌ | ❌ | ⬜ | **未实现** |
| 口令弱口令策略（禁止纯数字与常见口令） | ❌ | ❌ | ⬜ | **未实现** |
| 管理员二次验证 | ❌ | ❌ | ⬜ | 宪法第三章要求；**未实现**（未登记在门禁中，属缺口） |
...
| 规划输入的数据分级与 `ModelGateway` 闸门 | ❌ | ❌ | ⬜ | **未实现**（计划功能设计中已如实登记为偏差） |
...
| 攻击面八类检查**正式报告**（宪法第八章） | ❌ | — | ⬜ | **未产出**；实现过程中逐项覆盖过，但无报告 |
| 应用容器化（Dockerfile） | ❌ | — | ⬜ | **缺失**；仅有基础设施 `docker-compose.yml` |
...
| 项 | 实现 | 测试 | 验收 | 证据 |
| --- | --- | --- | --- | --- |
| 网页管理台（知识权限、公众号内容工作台、内容历史） | ✅ | ✅（Vitest） | ⬜ | `admin-web/` |
| 协同动态只读接口与任务权限过滤 | ✅ | ✅ | ⬜ | 门禁已勾选 |
| 协同动态表现层（静态列表） | ❌ | ❌ | ⬜ | **未做** |
...
1. **先把审计与登录限流分支合入 `main`**（本轮成果，439 项测试已绿），让门禁勾选状态与代码一致。
2. **容器化**：补应用 `Dockerfile`（宪法 2.4 要求）——这是 staging 部署的前置。
3. **补齐宪法硬缺口**：口令弱口令策略、管理员二次验证、登录限流的生产并发原子性验证。
4. **产出攻防八类检查正式报告**（宪法第八章，交付前必做）。
5. **拿到外部资源后**：跑 `py scripts/staging_preflight.py` → 跨租户测试 → 并发压测 → 沙箱验证 → 真实联调。
6. **客户端**：按 README 声明的三端形态推进（建议先做 PWA 审批提醒，再评估桌面端）。
