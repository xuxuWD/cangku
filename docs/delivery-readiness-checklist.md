...
| 测试文件数 | **103** 个测试模块（`tests/`；另有 `conftest.py` 与 `oidc_test_idp.py` 两个辅助文件，不计入） |
| 测试用例数 | **1036**（后端）+ 前端 `admin-web` 18、`companion-pwa` 24、`desktop` 13（Node `node:test`） |
