...
>
> **生成日期**：2026-09-10；**2026-09-11 同步三态**（第一次：容器化、弱口令策略、TOTP 二次验证、攻击面报告；第二次：数据分级闸门、协同动态表现层、运行指标采集、编排优化提案、GEO 阻塞更正；第三次：待办聚合接口、PWA 伴侣端、Electron 桌面端；第四次：死信通知渠道、并发探针、抓取器与发布器、门禁口径变更）
> **基线**：分支 `feature/planning-closure`，全量 **743 项测试通过**，`compileall` 退出码 0
>
> **证据口径（重要）**：本清单以 `docs/delivery-gates.md` 的勾选状态、`tests/` 目录的 85 个测试文件、`README.md` 的能力声明为准。标 ✅ 表示仓库内存在实现且门禁已勾选，**不等于我逐项重新验收过**；需要真实环境证据的项一律标 ⬜。
...
| 测试文件数 | **85**（`tests/`，不含 `conftest.py`） |
| 测试用例数 | **743**（后端）+ 前端 `admin-web` 18、`companion-pwa` 24、`desktop` 13（Node `node:test`） |

> **一句话结论**：服务端能力基本齐全，**代码侧的缺口只剩「内容安全评估用例」一项**；其余卡点集中在「真实环境验收」「外部系统契约（GEO）」与「桌面端打包签名 / PWA 真机」三处。
...
| 生产密钥轮换、真实统一登录验收 | ❌ | ❌ | ⬜ | **口径变更**：原「设备绑定」已从该项移除（产品采用「注册申请 + 管理员审批」制，理由见 `delivery-gates.md`「门禁口径变更记录」）；SSO/统一登录未实现，密钥轮换需部署密钥系统，两者均待外部资源（TOTP 二次验证已完成，见下两行） |
...
| 内容安全评估用例（提示词注入 / 数据泄露 / 越权输出） | ❌ | ❌ | ⬜ | **未实现**（项 9 剩余的代码缺口） |
...
| Electron 桌面端（README 声明为主员工端） | ✅ | ✅ | ⬜ | `desktop/`（Electron 安全壳 + electron-builder NSIS 配置）；`node --test` 13 项 + `test_desktop_assets.py` 9 项；**未签名、未打包、未做干净电脑测试** |
| 手机 PWA 伴侣端（审批与提醒） | ✅ | ✅ | ⬜ | `companion-pwa/`（登录、待办轮询、三类审批、manifest 与 service worker）；Vitest 24 项 + `test_pwa_assets.py` 8 项；**提醒是轮询而非 Web Push，真机安装未验收** |
...
9. **合并本批次分支 `feature/planning-closure` 回 `main`**（本轮成果：后端 743 项 + `admin-web` 18 + `companion-pwa` 24 + `desktop` 13 全绿）。
10. **内容安全评估用例**：项 9 剩余的**唯一代码缺口**（提示词注入 / 数据泄露 / 越权输出），可无需外部资源先行实现。
11. **拿到外部资源后**：跑 `py scripts/staging_preflight.py` → `py scripts/staging_concurrency_probe.py` → 跨租户测试 → 并发压测 → 沙箱验证 → 真实联调。
