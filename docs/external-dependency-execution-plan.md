...
- **契约索取**：`docs/geo-contract-request.md`（可直接发给 GEO 侧的索取表，含 7 类契约项与填写模板；已汇编本仓库与内部资料中的既有边界约束）。**能力到位前不改代码**。
- **执行序列**：
...
- **接入资料**：`docs/platform-account-onboarding.md`（平台 = **公众号**；含工作台发布协议、官方发布链路与硬约束、账号侧索取表）。**硬前置**：公众号账号主体类型与认证状态决定发布接口能否调用。
...
- **前置**：真实模型 `base_url`/`model`/`key`；平台凭据与发布授权；抓取白名单与合规确认；回执核对人。（平台侧账号权限与接入资料见 `docs/platform-account-onboarding.md`）
- **执行序列**：
