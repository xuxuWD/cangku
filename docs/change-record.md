...
| **未验证（不得读成已验）** | ① **真库未跑**：新增 `tests/test_commercial_lifecycle_postgres.py` 需 `WORKBENCH_TEST_DATABASE_URL`，本机无该变量 ⇒ **整体 skip**，PG 侧 `ORDER BY created_at, id` 的**真库行为未验证**（仅由 `tests/test_commercial_persistence.py` 的假连接 SQL 形状断言 + 内存实现用例覆盖）；且**该文件未纳入 `.github/workflows/ci.yml` 的 `postgres` job 命令**（该 job 当时只跑另两个真库模块）⇒ 真库路径当前**无人自动跑**。**（2026-09-15 销账：该文件已纳入该 job 命令，job 现逐个跑 3 个真库模块，口径变更见 [`delivery-gates.md`](delivery-gates.md)「门禁口径变更记录」2026-09-15 行；本条 ① 的"未纳入"与"无人自动跑"据此失效。真库行为本身仍以纳管后实跑为准，且**在 GitHub Actions 上尚未实跑**。）**② **「同刻多行」确定性**：构造用例是**同一 `requested_at` 值 + 不同 `id`**（内存用例 `job-a`/`job-b`；PG 用例同），断言按 `id` 升序；但**真库时间戳的实际精度/写入时刻不同**（PG `created_at` 由 `now()` 或写入值决定，可能带微秒）**未在真库验证**同刻是否真的并列 ⇒ 严格说「同刻」仅在**内存实现**下被确切构造与断言，PG 侧「同刻」**未验证**。③ **多副本无锁（N5）未测**：未起多副本 worker/beat、未构造并发窗口 ⇒ 登记项 ③ **未修、未验**。④ **多删除作业同一租户的新边界未测**：修 ② 后，若同租户存在多条待执行删除作业（`run_pending_jobs` 会因 `list_pending` 返回多行而**对同一租户多次调用 `execute_delete`**），首次执行使租户到 `DELETED` 后，后续调用将因 `DELETED → DELETED` 非法而被 `transition_tenant` **拒绝**（旧代码为静默重复置位）⇒ 该并发/重复场景**未加锁、未测**（与 N5 同族，如实登记，本轮不修）。 |
