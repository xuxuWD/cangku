...
| **验证（已做）** | ✅ 编排守护 **9 passed**；✅ **全量回归（含真库）⇒ 1901 passed**（上轮 1892 ＋ 本轮 9 条，无既有用例被跳过或删除）。✅ **反假两组真变红**：① 把 beat 合回 worker（worker command 加 `-B`）⇒ `test_worker_runs_celery_worker_without_embedded_beat` **1 failed**（`assert '-B' not in [...]`）；② 让 worker 的扫描间隔改为 `:-1800`（与 beat 的 `:-3600` 不一致）⇒ `test_worker_and_beat_agree_on_scan_interval` **1 failed**；两组均已还原并复绿。 |
| **未验证（不得读成已验）** | ① **真实镜像构建与容器启动未验收**（本机不跑该 compose：`wb-test-*` 容器组属另一项目；`docker compose config` 校验也**未跑**）⇒ `/var/lib/celery` 的「卷首次挂载继承镜像属主」与 `inspect ping` 健康检查**均为依据 Docker/Celery 既有语义的推断**，需在有 Docker 的环境实跑一次（`docker compose -f docker-compose.yml -f docker-compose.app.yml up -d` + `docker compose ps` 看 worker 健康 + `docker compose logs beat` 看 `Sending due task`）。② **prefetch/并发等生产参数未调优**（`--concurrency` 默认 2、无 `--max-tasks-per-child`）。③ **未接监控告警**（beat 挂了没人知道；runbook 未定义告警渠道）。✅ **compose 配置校验已做**：`docker compose -p <project> -f docker-compose.yml -f docker-compose.app.yml config -q` ⇒ **EXIT=0**（三服务解析正确、beat `--schedule` 与 `workbench-celerybeat` 卷渲染到位、`$$HOSTNAME` 以字面量语义保留；本机目录名含中文需显式 `-p` 指定 project name）。 |
...
| **未验证（不得读成已验）** | ① **客户侧 / 生产拓扑未验收**（本机 Docker Desktop + Windows 宿主；Linux 宿主、客户网络与凭据、容量与并发压测均未做）。② **未接监控告警**：beat/worker 挂掉仍无人知（runbook 未定义告警渠道；健康检查只到容器层）。③ **并发参数未调优**（`--concurrency` 默认 2、无 `--max-tasks-per-child`；prefork 下每个子进程各自建池 ⇒ 连接数上限 = 并发 × `max_size`，**未做容量测算**）。④ **`WORKBENCH_EMBEDDING_BASE_URL` 只验到「变量必填」**：本机演练填的是占位地址（`host.docker.internal:8899`，无服务），**记忆层真实调用未验证**（app 启动期不连 embedding，故健康通过）。⑤ 演练用镜像为本地构建（`workbench-app:latest`），**未推镜像仓库、未带版本号/构建编号**（「产物可追溯」在生产发布流程中仍需补）。 |
