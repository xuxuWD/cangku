...
| **未验证（不得读成已验）** | ① **WeKnora 直拉数据源未实现**（上游列表接口面未核实，规格 N1 已登记该缺口）；**「运维导出清单」的导出手段本身也未验证**——本机没有真实 WeKnora 实例，导出步骤是流程假设。② **未在任何真实存量租户上跑过导入**（本机演练用测试库 + 自造 2 条演示数据；生产/客户侧存量规模、超大批次、字段长度边界均未实测）。③ **CLI 在 Linux/容器内的行为未单独验证**（本机 Windows 跑通；CI 只跑 `compileall` 覆盖语法，不跑该脚本）。 |
