...
| **未验证（不得读成已验）** | ① **真实 Celery beat 联调未做**：本机与 CI 均**未起 worker/beat 进程**，只验证了「排程键与任务函数 + 接线后真跑服务（内存仓储）」；staging 上 `celery beat` 的实际周期触发**未取证**。② **PG 跨租户扫描的规模表现未测**：候选查询为 `status + review_due_at` 条件，**现有索引 `(tenant_id, status, review_due_at)` 对无租户谓词的扫描不可用 ⇒ 实为顺序扫描**；治理表为元数据级（每文档一行），当前规模无碍，**大表表现未验证**（若后续量级上升需补 `(status, review_due_at)` 索引并走新迁移）。③ **N6 未决**（首轮到期缺口）：`publish` 不设 `review_due_at` ⇒ 未复核过的文档不会被 beat 捞到；已登记待裁决（A 发布即置 due / B 维持现状），**未擅自改**。 |
