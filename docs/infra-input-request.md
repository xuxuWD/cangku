...
**发出人自查（三条红线）**：① 附言与文档全文均不含密钥 / 口令 / DSN（红线见 §3）；② 回执只填在 §2 表内、按「脱敏值」口径回传；③ 回执到达后**先做脱敏检查**（判定口径：以 §2 表逐项填写回传为准，「材料已收到」类确认条**不构成**回执；表内若现口令 / 密钥 / DSN / 令牌**原文** → 不录入任何真源，按 §3.2「已泄露」处理——**更换密钥，而非删掉那一行**），**再登记**（真源回写 → 提交推送），最后由我方按 runbook 执行——**不因催办而压缩授权流程**。