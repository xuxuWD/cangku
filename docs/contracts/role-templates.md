...
> 来源（**代理检索，未逐字复核**）：`C:\Users\浮生\.evoflow\knowledge\vaults\evoflow-user-guide\explanation\smart-employees.md`（岗位合同要素）与其 `cases\smart-employee-playbooks.md`（18 类岗位画像，可作模板参考）、`guides\chat\preset-roles.md`（工具白名单最小权限）。
