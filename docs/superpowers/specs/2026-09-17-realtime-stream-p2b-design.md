...
> **日期**：2026-09-17
> **状态**：**已评审（2026-09-17，用户批准本文件）**；§5 待裁决**按推荐执行**（Q7 = 候选① `messages:stream`；Q8 = 7 天保留 + 双上限熔断 + 6h 悬挂兜底 + 逐租户清理；推送底座 = PG 表 + 短轮询；审批后推进的过程事件本期不做（**2026-09-17 回改：已由 P2c-2 纳入**）；借鉴第 6/7/8 条不纳入本期）。实现按 §7 实施顺序推进；**未经用户确认不提交、不推送**。**2026-09-17 回改**：「不落原文」修订为有界摘录、分享与多端协同纳入 P2c-6（详见 §1.3 / §5 / §6 回改留痕）。
...
8. **不做流历史回放 UI / 不做跨端同步**：帧到期即不可查（与运行事件同口径）；~~不做会话分享与多端协同~~（**2026-09-17 回改：分享与多端协同已纳入 P2c-6**；「不做实时在线态」保留）。

> **回改留痕（2026-09-17 · P2c 二次裁决 / P2c-2 契约修订）**：
> ① **第 6 条修订**：「不落原文」修订为**有界摘录**通道（执行输出 / 文件变更；默认 16 KiB、二进制只留 `bytes`+`sha256`、过 `redact_payload`、**不进审计 / 不进消息表**、随帧 7 天）——契约落点：`api-contract.md`「内容级回传」（P2c-2）；`027` 密文仍是**唯一**受控例外。
> ② **第 8 条推翻**：会话分享（本租户指定用户 · 只读）与多端协同（受邀成员可发言）**已纳入 P2c-6**。
> ③ **§5 待裁决项「审批后推进的过程事件」推翻**：已由 **P2c-2** 纳入（同一 run 续写 `seq`、终态先重开、不新增 `kind`）。
> ④ **只增不改**：帧 `kind` 取值域、`X-Stream-Run-Id`（`messages:stream` 既有）与既有端点行为**零破坏**；SSE 读端仅**只增**响应头 `X-Stream-Run-Id` 与帧内 `run_id`。
...
2. **默认只落摘要**：`tool.result` 帧只含 `status` + 摘要 + `args_digest` + `sha256`（与 `027` 落库口径一致）；**不含** stdout 全文、文件正文、宿主真实路径（对外虚拟路径）、凭据 / 认证头 / Cookie。**（2026-09-17 回改：修订为「有界摘录」——`tool.result` **只增** `output_excerpt` / `output_truncated` / `output_bytes` / `file_changes`；仍不落全文与二进制；契约见「内容级回传」）**
...
| —   | **审批后推进的过程事件**                    | ~~本期不做~~（**2026-09-17 推翻：已由 P2c-2 纳入**——同一 run 续写 `seq`、终态先重开、不新增 `kind`）         | 纳入本期（需把推进路径也接入帧写入）         |
...
1. **本规格后端已交付**（迁移 `036` + 帧仓储 / 写入网关 + `messages:stream` + SSE 读端点 + 清理任务）；前端消费属 P2c；**其余登记项**（反代 / 压测 / 多副本 / staging 校准）仍为**未验证**。
...
5. **`027`** **加密正文列与帧的关系**：本层不触碰（只落摘要；**2026-09-17 回改：`tool.result` 只增「有界摘录」字段，密文列仍不外溢到帧**）；「待批动作重跑后产生的过程事件」**已由 P2c-2 纳入**（**实现完成前不得声称覆盖**）。
6. **回改（2026-09-17 · P2c 二次裁决 / P2c-2）**：本层「不落原文」修订为**有界摘录**（契约「内容级回传」）；「审批后推进」推翻并由 P2c-2 承接；会话分享与多端协同纳入 **P2c-6**；**帧体积与熔断触达率的量化登记在 P2c 规格 §6-6**（本文件不再单列）。
