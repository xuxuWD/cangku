...
| **CI** | `push` 触发（run `34685181370`） | **4 个 job 全绿**：后端（pytest + compileall）/ 网页管理台 / 手机伴侣端 / 桌面端 |

### 8.5 本段仍未验证（如实登记）

1. **staging / 生产未验收**（同 §8.3 末条）。
2. **浏览器人工闭环未做**：常驻外壳只做了自动化测试（草稿保留、只切显隐、未访问不挂载、四个 URL 直达），**没有真人点过 12 个页面的视觉与滚动表现**；`.view-slot` 的布局等价性是按 CSS 推导的，存在未观测到的视觉回归风险。
3. **授权闸门拦不到真实副作用**（段一没有真实工具，§2.3 已登记）。
4. **只登录一个消费点**：`needs_approval` 目前只在「任务创建」被调用；对话入口、工具层、记忆层的判定分别在 P2b / 段二 / P3。
