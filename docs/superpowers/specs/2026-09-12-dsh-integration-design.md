...
| **异常与非法输入** | 路径逃逸（`..`、符号链接、硬链接）、黑名单命令、包装器间接命令、工作卷同名可执行文件、伪造参数、未知字段、跨租户、`agent_key` 非法、容器逃逸尝试（挂 `docker.sock`、`--privileged`）、**`artifact.export` 跨租户/越权导出（用例 26）**、**无 `Idempotency-Key` 时的行为（用例 14②）** |
