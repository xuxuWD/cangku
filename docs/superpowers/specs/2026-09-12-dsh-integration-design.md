...
| 29 **【⚠️ 未实现·待办，见 §8 U18】** | **`actions` 缺省语义 + `ToolAction` 字段**（**R5-4 新增**）：① `actions` 缺省且在「⑤ 判定无需审批」路径 → **只退化为运行级摘要比对**；**〔2026-09-13 用户裁决：原「该路径不得触达 ⑧」作废，该路径完成 ②③④ 后进入 ⑧〕**（打桩断言：执行器**被调用一次**，且 ②③④ 的判定留痕存在）；② `actions` 缺省却处于需审批路径 → **必须 `409`**；③ `ToolAction` 8 字段**逐个非空**，`args_digest` 与 §4.1.4 算法一致，且**投影中不含参数原文与 `args_json`** | 边界 | P0 | 把缺省实现为 `run_records is None → return`（fail-open）→ 必须红（§3.2 明令禁止）；把缺省当作「跳过 ⑥⑦ 直接执行」的开关 → 必须红 |
...
| U18 | **§5 用例 29（P0）未实现 —— `actions` 缺省语义与 `ToolAction` 投影**（2026-09-13 新增；随九步闸门实现验收一并登记） | **现状**：段二-2 的**九步闸门 ①–⑨ 已实现**且回归全绿（1526 passed / 10 skipped），但 **⑦ 目前只调用既有签名的 `ensure_execution_authorized`**，**`actions` 参数与其缺省语义尚未引入** ⇒ 用例 29 的 **②③ 无从实现**；**① 的"打桩断言执行器被调用一次"亦无对应用例**（测试替身只有 `audit.calls` / `authorizer.calls` 计数器，**执行器自身没有调用计数**，故该断言当前**不可判定**）。**收口方式**：① 为 `ensure_execution_authorized` 增 `actions` 参数并定死缺省语义（§4.1.7-5）；② 给假执行器加调用计数；③ 补用例 29 ①②③ 三条断言与反假。**在此之前不得声称「§5 用例已全绿」。** |
