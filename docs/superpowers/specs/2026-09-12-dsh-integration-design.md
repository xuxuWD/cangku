...
- **本段不新建表**（X5 **已定**，2026-09-12）：工具调用进既有审计（append-only，明细走 `ALLOWED_DETAIL_KEYS` 白名单）；授权位已有迁移 `026`。**不建「工具调用记录表」**——避免出现第二事实源与额外迁移。
...
- 一个**外置配置开关**可把运行时整体切回 `MockRuntime`；**切回后全量用例必须仍全绿**（当前基线：后端 **1434**（2026-09-12 CI 实测）/ 管理台 147 / 伴侣端 37 / 桌面 19；后三项为文档原值，本次未复核）。
- 本段不做「灰度」；灰度属 P6 口径。
...
| 项 | 内容 |
| --- | --- |
| 迁移 | **无新迁移**（X5 **已定**：不建工具调用表；授权位 `026` 已有，工具调用走既有审计） |
