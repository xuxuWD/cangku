...
| U21 | ✅ **已闭环（2026-09-14 实现；下方登记一条未验证项）—— 跨进程权威同步** —— 原「❌ 未闭环（2026-09-14 新增 —— 从 U20 残余② 单列）」 | **缺口**：②④ 的权威数据（令牌自持绑定 `TokenBindingStore` 与「当前执行」登记表 `ActiveExecutionRegistry`）**都是进程内对象**；而生产部署下**签发令牌的是工作台进程、做校验的是回调边车（另一进程）** ⇒ 两进程**不共享**这些状态 ⇒ **边车独立启动时会把全部回调请求拒绝**（方向是 **fail-closed、不会误放行**，但**功能不通**）。**⇒ 后果**：②④ 的**机制、端点、判定、取证都已就位**，但**"生产拓扑下真正跑通"缺这一环** ⇒ **在此之前，不得声称「②④ 已强制」**（与 §U20 同一禁令）。**裁决（2026-09-14，用户）= 候选②「边车纯转发 + 判定回工作台」**：
  ...
  - **〔2026-09-14 落地（本轮返工）〕** **已实现**：① 边车**去状态** —— 移除 `TokenBindingStore` / `ActiveExecutionRegistry` 的使用（`app/exec_callback/registry.py` 删除，登记表迁至 `app/tool_execution/active_execution.py`），边车退化为**无状态纯转发**（源码内**无**任何绑定比对原语，AST 断言守护）；② **②④ 判定回到工作台** —— 新端点 `POST /api/v1/internal/exec-callback`（`app/tool_execution/callback_guard.py`）：`expected` 从**工作台权威状态**重建、`constant-time` 比对、不匹配 / 跨租户 / 旧代次 / 未知令牌 → `403`（不泄露存在性）+ 记审计（**复用 `tool.blocked`，未新增动作码**）；③ **专用回调网** `workbench-exec-callback`（只含「工作台 + 边车」）：**边车双宿**、**执行容器不可达回调网**（新增断言）；④ **鉴权 = 预共享密钥**（`WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`，恒时比对、**缺失即拒**）、**限流 = `100` rps**（超限 `429`）。**取证**：`tests/test_exec_callback.py`（工作台六条 + 边车纯转发 + 单进程端到端 + **反假 ≥3 组**）、`tests/test_exec_callback_network.py`（真容器：可达边车、不可达工作台、**不可达回调网**）。**未验证（不得读成已验）**：① **两进程真实拓扑下的成功路径** —— 本轮只做到**单进程内两监听**（uvicorn 同进程另起真 HTTP，边车 → 真工作台），**未经两个独立进程 / 两个容器实跑**；② **`generation` 真实多 turn** —— 仅单测覆盖登记表单调性与既有代次，**未经真实 dsh 多 turn 驱动**；③ **限流是否全局口径** —— 实现为**进程内令牌桶**，多副本下是「**每副本** 100 rps」而非全局 100 rps（**须与"100 rps"的原意确认是否一致**）；④ 真实 dsh turn 触发的回调（触发方仍不存在）。 |
