...
| U19 | **`resume` 签名缺 `actor` / `plan` ⇒ 重跑路径无法执行「运行级」授权闸门**（2026-09-13 新增；随决议端点接线验收一并登记） | **暴露经过**：把 `tool_execution_service` 接入决议端点时，严格照 §4.1.6-4 的调用链实现 `resume(run_id, approval_id)` —— **该签名不含 `actor` 与 `plan`**，而 ⑦ 的运行级闸门 `ensure_execution_authorized(actor, run_id, plan, actions)` **必须有这两者** ⇒ **审批后重跑时只做逐行校验，运行级摘要比对缺失**。**当前影响有限**（逐行校验仍在，且 `027` 是唯一授权权威），但**与 §3.2 ⑦「计划摘要 **+ 参数摘要** 比对」的字面要求不一致**。**收口方式（二选一）**：(a) 给 `resume` 增 `actor` / `plan`，或由服务端从 `run_records` / `027` 行**回查**（**不得由调用方任意声称**）；(b) 在 §4.1.6-4 明确**豁免**「重跑路径不做运行级比对」并写明理由。**在此之前不得声称「⑦ 的完整口径已实现」。** |
