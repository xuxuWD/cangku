...
- **dsh 进程位置与模型凭据来源（P1，2026-09-13 已定口径 —— 裁决路径①「自建模型网关」）**：实测确认**模型调用由容器内的 dsh 进程自己解析并发出**（剖面含 `dsh-llm`/`dsh-agent-default-model`；请求上下文含 `provider`/`model`，错误回传带 `code=AUTH`/`status=401`），且**SDK 握手不接受任何凭据字段**、`dsh-credentials-local` 明写预期密钥经**容器 `-e`** 传入（门禁 §F8.1）。**故必须新增「模型网关」组件**，口径如下：
  ...
  3. **短期令牌六条（硬要求）**：① **每 turn 新铸**；② **绑死租户 / 会话 / 代次**；③ **服务端为准**（不信任容器内声明）；④ **constant-time 比对**；⑤ **终态同步吊销**；⑥ **孤儿上限**。**禁止**任何"开机固定的共享令牌"。
   ...
   - **落点与时机** ＝ **实现挂在段二-4 的对话入口 / 回调面**（2026-09-13 用户裁决选 (a)）—— 因为规格所述的两个回调场景（**导出产物**、**请求授权**）**当前都不存在**：`artifact.export` 是**自建工具、不经容器**（§3.1.1 注 2），对话入口属段二-4。**⇒ 当前无调用方，故不实现**（避免造出"没人调用的校验"—— `/__revoke` 的前例）。**〔2026-09-13 更新〕段二-4 已交付，但其实测确认「执行侧回调面」**仍不存在**（无 tool-call 分派端点、无 export 回调路由；§2.3 亦排除 dsh 反向请求）⇒ 本条的**落点问题并未解决**，已单列为 §8 **U20**。**判据见 §5 **用例 35**。**在实现之前，不得声称「②④ 已强制」。**
...
| U15 | **丙案机制的落地**（受控正文密文列 + `param_roles` 声明字段） | **文档层已实施（第七轮补丁后更完整：新增 `BodyCipher` 组件落点、`WORKBENCH_BODY_ENCRYPTION_KEY`/清理周期两项配置、加解密失败语义、密钥轮换窗口、`expired` 执行方、用例 32–34）**。**实现现状（2026-09-14 更正：原文「仍未做」的三项已不成立，逐项如实）**：① **`migrations/027` ✅ 已落地**（`migrations/027_dsh_tool_execution.sql`；静态契约测试 `tests/test_dsh_tool_execution_migration.py`，并有**真库演练**——`tests/test_tool_action_store_postgres.py` / `tests/test_dsh_execution_postgres.py` 在带 `WORKBENCH_TEST_DATABASE_URL` 的一次性 PG16 容器上通过；已登记 `.env.staging.example`）；② **`BodyCipher` 加解密 ✅ 已实现并有测试**（`app/tool_execution/body_cipher.py` 的 AES-256-GCM 封套 `nonce(12B)‖ciphertext‖tag(16B)` + `tests/test_body_cipher.py`；装配期经 `BodyCipher.from_base64` 校验密钥格式，缺失 / 非法即拒绝启用真实执行）；③ **TTL 清理任务 ✅ 已实现**（`app/tool_execution/store.py` 的 `expire_pending_bodies` **InMemory + Postgres 双实现** + `app/tool_execution/cleanup.py` 的 `BodyCleanupTask`：**启动时 + 按 `WORKBENCH_BODY_CLEANUP_INTERVAL_SECONDS` 周期**、跑在 API 进程内、与孤儿清扫**同处注册**；**只清未判决的到期行，未到期的绝不动**，重复执行幂等；清理失败记 `error` **告警并计数**，**不新增审计动作码、不落正文**）；④ **`expired` 状态迁移 ✅ 已实现（含审计证据）**（到期 `pending` 行**同一事务**置 `status='expired'` + `decided_by="system:approval-timeout"` + `decided_at=到期时刻` + `decision_source="timeout"` + 清空 `body_ciphertext`/`body_expires_at`，与 `027` 既有的 `decision_check` / `body_check` **相容**，未改 `027`）；**并同时留下审计证据**——按**既有**动作码 `run.approval_decided`（**未新增动作码**、不含正文，§4.1.7-6「决议事实由既有 `run.approval_decided` 表达」）为每个到期行写 `status="expired"`、`actor_id="system:approval-timeout"`（受控常量）、`target=run`）；⑤ **`agent_key` 校验收紧 ✅ 执行入口生效**（落点 = `app/conversation/execution.py::_governance` → `app/workforce/store.py` 的 `read_agent_governance` 补齐**岗位停用连带**：员工 `active` **且**所属岗位 `active` 才可用；返回 `None` 即 `422`，不创建承载任务 / 运行 / 幂等行）。**差异登记（避免下一个人误判，2026-09-14）**：契约 `:644` 与前置清单 §B14 点名"复用 `ensure_agent_binding_available`"，本实现**改用 `read_agent_governance`**——两者**可用性判据同口径**（都 = 员工 `active` **且**岗位 `active`，因 `ensure_agent_binding_available` → `agent_is_active`）；**差异**：① `read_agent_governance` **不要求 `super_admin`**（`agent_is_active` 经 `_ensure_admin` 要求），执行入口操作者可为任意可执行角色；② 以 `None` 表达「不可用」（而非抛 `DirectoryNotManaged`），且返回治理两字段（而非归一键）。语义仍等于「存在且启用」，故收紧成立、**不放宽**。**仍未做 / 未验证**：**密钥轮换（双密钥窗口，§3.4 约束 1）未实现**；§5 用例 32③（审批落定同事务清空）与 33（各通道正文检索）的其余判据**尚未逐条补齐**；③④ 的 Postgres 分支只在**带 DSN 的一次性容器**中验证（`tests/test_tool_action_store_postgres.py` 默认 skip，**非 CI 常驻守护**）；**④ 的审计（`run.approval_decided`）只在 InMemory 分支有测试守护，Postgres `RETURNING` 路径未跑 → 未验证**；**多副本并发跑清理的重复/幂等取证未做**；`027` 的**升级/回退演练**（§4.1.5 回退四步）**仍未验证** → 未取证部分**不得视为可用** |
...
| U20 | **②④ 控制面强制校验：组件已交付，但「执行侧回调面」不存在 ⇒ 未挂载**（2026-09-13 新增；用户裁决方案 (c) 维持现状） | **现状**：②④ 的口径已定死（§3.5 P1-3：工作台侧自持绑定 + `constant-time` + **`generation` = 会话内 turn 序号**）且**组件已实现**（`app/tool_execution/token_binding.py`，段二-4 已接线），**但没有任何调用方**。**实测确认（2026-09-13）**：① 无**工具调用分派**端点（`tool_call_id` 仅是消息 / 运行记录的字段）；② 无 `/export`、`/artifacts` 之类回调路由；③ 规格 **§2.3 已排除 dsh 发起反向请求**（只有 `initialize` / `session.prompt` / `shutdown` + 4 通知，**无审批方法**）⇒ **「请求授权」不可能由 dsh 发起**。**原定落点（"挂段二-4 的对话入口"）不成立** —— 那是**用户入口**，调用方**不是"执行侧"**。**⇒ 结论：无可挂的调用方，故未挂载**（避免造出"没人调用的校验"—— `/__revoke` 的前例）。**待办与解锁条件**：唯有**造出真实回调面**（例如让「导出产物」走**容器 → 工作台的 HTTP 回调**、随行短期令牌）才有落点；**但该做法会改变 §3.1.1 注 2 的既有落点**（`artifact.export` 现为"我方自建工具、**不经容器**"）⇒ **属地基级改动，须先出范围说明再动**。**在挂载并取证之前，不得声称「②④ 已强制」**（判据见 §5 **用例 35**）。 |
