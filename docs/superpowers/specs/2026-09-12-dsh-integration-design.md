...
| **新增 23 项**配置（2026-09-13 更新：原 12 项 + **路径①「模型网关」六项** + **⑥ 孤儿上限 `WORKBENCH_EXEC_ORPHAN_LIMIT`**；**2026-09-14 更新：+ 执行回调边车四项 `WORKBENCH_EXEC_CALLBACK_*`（含 §8 U21 新增的预共享密钥），共 19 → 23 项**） | `tests/test_env_templates.py` | 扩该守护测试至覆盖全部新增项（见前置清单 §B15，**须同步改为 23 项**） |
...
### U22 密钥轮换的**触发方式**与**旧密钥配置落点** — ❌ **未闭环（2026-09-14 新增）**
...
**🔴 缺口（真源未定义，本轮按令未自造）**：① **轮换的触发方式** —— 规格通篇只有"双密钥窗口 / 新加密旧解密 / 窗口长度"三句，**未定义谁在何时触发轮换**（无时间戳、无开关、无运维流程口径）；② **旧密钥的来源与配置落点** —— §4 的 23 项配置中**只有一把** `WORKBENCH_BODY_ENCRYPTION_KEY`，且 §3.4 R3 把其格式**定死为单值** base64（32 字节原始密钥）；**没有任何"上一把密钥"的配置名 / 格式 / 存放与销毁口径**。

**⇒ 后果**：**双密钥机制的"能"已具备，但生产上无人能把旧密钥交给进程** ⇒ 轮换**事实上无法执行**。**在补齐之前，不得声称"密钥轮换已就绪"。**

**解锁条件（候选，需裁决）**：① 新增配置项（如 `WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`，多值/JSON 数组）承载旧密钥 ⇒ 须同步**三处台账**（`app/settings.py` / `.env.staging.example` / `tests/test_env_templates.py`）+ **§B15 计数**；② 明确定义**触发方式**（时间戳比对？运维显式动作？启动时按配置判定？）与**窗口结束后的销毁口径**。**判据**：一次性"轮换前加密 → 提供旧密钥 → 成功解密"的端到端证据；且**窗口外旧密钥必须失效**（解密失败须 fail-closed）。
...
**解锁条件（候选，需裁决）**：① 对话入口**不再把原始调用 JSON 落消息表**（改为落"已脱敏的引用/摘要"）；② 或**修正 §3.4 约束 3 的持久层清单**（把消息表纳入并规定其 `body` 处理口径）；③ 或**修正 32②(c1)③ 判据**（明确消息表允许保留用户原文及其边界）。**判据**：真库上对 `workbench_conversation_messages.content` 做 32②(c1)③ 的检索断言（**须在验收记录写明库与查询**）；**不得**用 `args_json` 单点代表全部三处。
