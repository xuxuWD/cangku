...
- **dsh 进程位置与模型凭据来源（P1，2026-09-13 已定口径 —— 裁决路径①「自建模型网关」）**：实测确认**模型调用由容器内的 dsh 进程自己解析并发出**（剖面含 `dsh-llm`/`dsh-agent-default-model`；请求上下文含 `provider`/`model`，错误回传带 `code=AUTH`/`status=401`），且**SDK 握手不接受任何凭据字段**、`dsh-credentials-local` 明写预期密钥经**容器 `-e`** 传入（门禁 §F8.1）。**故必须新增「模型网关」组件**，口径如下：
  ...
  3. **短期令牌六条（硬要求）**：① **每 turn 新铸**；② **绑死租户 / 会话 / 代次**；③ **服务端为准**（不信任容器内声明）；④ **constant-time 比对**；⑤ **终态同步吊销**；⑥ **孤儿上限**。**禁止**任何"开机固定的共享令牌"。
   ...
   - **落点与时机** ＝ **实现挂在段二-4 的对话入口 / 回调面**（2026-09-13 用户裁决选 (a)）—— 因为规格所述的两个回调场景（**导出产物**、**请求授权**）**当前都不存在**：`artifact.export` 是**自建工具、不经容器**（§3.1.1 注 2），对话入口属段二-4。**⇒ 当前无调用方，故不实现**（避免造出"没人调用的校验"—— `/__revoke` 的前例）。**〔2026-09-13 更新〕段二-4 已交付，但其实测确认「执行侧回调面」**仍不存在**（无 tool-call 分派端点、无 export 回调路由；§2.3 亦排除 dsh 反向请求）⇒ 本条的**落点问题并未解决**，已单列为 §8 **U20**。**判据见 §5 **用例 35**。**在实现之前，不得声称「②④ 已强制」。** **〔2026-09-14 实现更新 —— §8 U20 方案 b-1「边车单端口回调面」〕** **真实落点 ＝ 执行回调边车**（`app/exec_callback`：`POST /internal/exec-callback`）。边车是**独立进程**、**只监听一个端口**、**只挂 `workbench-exec-internal`**（**不双宿工作台默认网络**）；②④ 判定在**边车内（工作台控制面）**完成——`expected` **从服务端权威上下文重建**（`ActiveExecutionRegistry` 的「会话当前代次」，**绝不取自请求体**），与令牌自持绑定做 `constant-time` 比对，不匹配 / 跨租户 / 旧代次 → `403`（不泄露存在性）+ 记审计（复用 `tool.blocked`，不新增动作码）。放行后经 **`边车 → 工作台`** 的**受控出向调用**交回工作台（容器**不直连**工作台）。**判据已从纯内存升级为真端点端到端**（`tests/test_exec_callback.py`，含反假）；**残余未验证项见 §8 U20。** **〔2026-09-14 二次更新 —— §8 U21 裁决「候选②：边车纯转发 + 判定回工作台」〕** 上述「判定在边车内」的落点**已被返工**：边车改为**无状态纯转发**（不再持有 `TokenBindingStore` / `ActiveExecutionRegistry`，不再比对令牌 / 绑定），**②④ 判定回到工作台**——新端点 **`POST /api/v1/internal/exec-callback`**（`app/tool_execution/callback_guard.py`）从**工作台权威状态**重建 `expected`（`ActiveExecutionRegistry` 的会话当前代次，**绝不取自请求体**）并与令牌自持绑定做 `constant-time` 比对；边车**双宿** `workbench-exec-internal` + **回调网**（`workbench-exec-callback`，只含「工作台 + 边车」），执行容器仍**只挂执行内网**（**不可达工作台、不可达回调网**）。**边车 → 工作台**的鉴权 = **预共享密钥**（`WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`，对称、**缺失即拒**）、限流 = `100` rps（超限 `429`）。**取证**：`tests/test_exec_callback.py`（工作台六条 + 边车纯转发 + 单进程端到端 + 反假 ≥3 组）、`tests/test_exec_callback_network.py`（真容器：执行容器**可达边车、不可达工作台、不可达回调网**）。
  4. **网关职责**：只做**凭据注入 + 转发 + 计量/限流 + 审计**；**不得**改写模型语义；**超时/重试上限必须显式配置**（归口在此，不在容器内）。
  5. **网络面**：容器**无外网出口**，**仅可达网关与执行回调边车**（均在同一内网桥，§3.3）。原"`--network none` 下 turn 仍成功"的口径**正式作废**——它与 dsh 架构不相容；**改判为 C′：容器内不存在指向供应商域名的连接**（见门禁 §B14）。**〔2026-09-14 更新（§8 U20 方案 b-1）〕** 回调边车是**第二个**允许的内网目的地（`POST /internal/exec-callback`）；**判据 C′ 的证据形态随之变化** —— 允许的连接从「只有一条到网关」变为「到网关 + 到回调边车」，门禁 §B14 判据 C′ 已同步改写。**〔2026-09-14 二次更新（§8 U21 裁决「候选②」）〕** 边车改为**双宿**执行内网 + **回调网**；回调网**只含「工作台 + 边车」**，**执行容器仍不可达回调网**（新增断言见 `tests/test_exec_callback_network.py`）；**②④ 判定在工作台**，边车**无状态纯转发**。
...
| 新增配置（外置） | `WORKBENCH_AGENT_RUNTIME_BACKEND`（默认 `mock`）、`WORKBENCH_EXEC_IMAGE_DIGEST`（**digest，非 tag**）、`WORKBENCH_EXEC_WORKSPACE_ROOT`、**`WORKBENCH_EXEC_TRUSTED_ROOTS`（重审补：受信任且不可写的可执行根，默认 `/usr/bin`）**、`WORKBENCH_EXEC_TIMEOUT_SECONDS`（**默认 `180` 秒** —— 2026-09-13 定值，见 §8 U11）、`WORKBENCH_EXEC_PIDS_LIMIT`（**默认 `256`**）、`WORKBENCH_EXEC_MEMORY_MB`（**默认 `2048`**）、**`WORKBENCH_EXEC_CPU_QUOTA`（重审补：宪法部署检查要求 CPU 上限；**默认 `2.0`**，对应 `docker --cpus`）**、**`WORKBENCH_EXEC_ORPHAN_LIMIT`（**⑥ 孤儿上限；**默认 `8`**；超限 → 拒绝新执行并告警（fail-closed，不打挂进程）；清扫 = 启动时 + 按 `WORKBENCH_BODY_CLEANUP_INTERVAL_SECONDS` 周期，跑在 API 进程内 —— 见 §8 U17）**、`WORKBENCH_DSH_VERSION`（精确版本）、`WORKBENCH_ARTIFACT_EXPORT_ENABLED`（**默认 `false`，fail-closed**：关闭时 `artifact.export` 不装配，请求被拒并记审计）、**`WORKBENCH_BODY_ENCRYPTION_KEY`（P3 新增：正文密文密钥；**必填且非空**；**格式 = 32 字节原始密钥的 `base64` 编码**（启动期校验解码长度，见 §3.4 约束 1 / R3）；配置外置、不进仓库；**不得复用** `backup_encryption_key`）**、**`WORKBENCH_BODY_CLEANUP_INTERVAL_SECONDS`（P3 新增：TTL 清理周期；**默认 `60` 秒** —— 2026-09-13 定值，见 §8 U16；清理在**启动时 + 按此周期**执行，见 §4.1.5）**、**`WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`（2026-09-14 §8 U22 新增：**旧正文密文密钥（多值）**；格式与主密钥同口径 = **逗号分隔的 `base64`(32 字节原始密钥)**（真源未规定多值格式 → 取既有风格；见 §8 U22）；**进程启动时读入** ⇒ 轮换 = 改配置 + 重启（**无热轮换**）；窗口结束后由运维从配置移除 + 重启（进程不再持有该旧密钥）；**缺失 / 留空 ⇒ 不使用旧密钥**（fail-closed）；配置外置、不进仓库）**、**路径①「模型网关」六项（2026-09-13 裁决路径①新增，语义见 §3.5 P1 段；**§F8.6 第 6 项的"待补"就此闭合**）**：`WORKBENCH_MODEL_GATEWAY_BASE_URL`（**容器内 `baseURL` 指向的网关地址**，须在该内网桥内可达；**供应商域名不得出现在本项**）、`WORKBENCH_MODEL_GATEWAY_TOKEN_TTL_SECONDS`（**短期网关令牌有效期**；**默认 `300` 秒**；**每 turn 新铸**；**须 ≥ `WORKBENCH_EXEC_TIMEOUT_SECONDS`（§8 U11）**——两者默认值**已于 2026-09-13 一并定死**）、`WORKBENCH_MODEL_GATEWAY_UPSTREAM_BASE_URL`（**上游供应商端点**——**供应商域名只允许出现在网关侧/宿主侧**，**不得进入容器**）、`WORKBENCH_MODEL_GATEWAY_UPSTREAM_API_KEY`（**上游供应商密钥**：**必填非空、配置外置、不进仓库、不进容器**（env / argv / 镜像层 / 日志）；**启动期做与 `WORKBENCH_BODY_ENCRYPTION_KEY` 同口径的非空校验**，缺失则**拒绝启用真实执行并告警**）、`WORKBENCH_MODEL_GATEWAY_UPSTREAM_TIMEOUT_SECONDS`（**网关→上游超时**；**默认 `60` 秒**；**必须显式配置**，见 §3.5 P1 第 4 条）、`WORKBENCH_MODEL_GATEWAY_MAX_RETRIES`（**默认 `0`，fail-closed**：不隐式重试）**、**执行回调边车四项（2026-09-14 方案 b-1 新增、同日 §8 U21 返工，语义见 §3.5 P1 第 3 条 / §8 U20 / §8 U21）**：`WORKBENCH_EXEC_CALLBACK_LISTEN_HOST`（**边车唯一监听地址**，默认 `0.0.0.0`；对外可达性由「边车**双宿**执行内网 + 回调网」限定）、`WORKBENCH_EXEC_CALLBACK_LISTEN_PORT`（**边车唯一监听端口**，默认 `8081`；**不得再开第二个端口 / 第二条路由**）、`WORKBENCH_EXEC_CALLBACK_FORWARD_URL`（**边车 → 工作台的受控出向调用目标**；**默认留空 = 不启用边车**（fail-closed）；**不得把回调地址烤进镜像**）、**`WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`（2026-09-14 §8 U21 新增：「边车 → 工作台」**预共享密钥**，对称鉴权；**必填非空、缺失即拒绝启动 / 拒绝请求**（fail-closed）；来自部署密钥系统、不进仓库、**不得烤进镜像**）** → **共 24 项**。**落点状态（2026-09-14 §8 U22 后）**：**24 项已全部进 `app/settings.py`**（含类型、默认值与 `ge/le` 边界；**不设裸名别名**，只认 `WORKBENCH_` 前缀）、**已全部进 `.env.staging.example`**，并由 **`tests/test_env_templates.py` 三个守护用例**覆盖（字段存在性 / 模板全覆盖 / 默认值钉死 + `TTL ≥ EXEC_TIMEOUT` 硬约束），**含反假测试**；⚠️ **"必填非空 / `base64` 32 字节"的启动期校验未实现**（归 §4.1.6-3 装配期）——见门禁 **§B15** |
...
| **新增 24 项**配置（2026-09-13 更新：原 12 项 + **路径①「模型网关」六项** + **⑥ 孤儿上限 `WORKBENCH_EXEC_ORPHAN_LIMIT`**；**2026-09-14 更新：+ 执行回调边车四项 `WORKBENCH_EXEC_CALLBACK_*`（含 §8 U21 新增的预共享密钥），共 19 → 23 项**；**2026-09-14 §8 U22 更新：+ 旧正文密文密钥 `WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`，共 23 → 24 项**） | `tests/test_env_templates.py` | 扩该守护测试至覆盖全部新增项（见前置清单 §B15，**须同步改为 24 项**） |
...
### U20 ②④ 控制面强制校验（执行侧回调面）— ✅ **已闭环（2026-09-14，方案 b-1「边车单端口回调面」）**，残留在下

**原登记（2026-09-13）**：②④ 控制面强制校验 —— 组件已交付，但「执行侧回调面」不存在 ⇒ 未挂载（原用户裁决方案 (c) 维持现状）。

**现状**：②④ 的口径已定死（§3.5 P1-3：工作台侧自持绑定 + `constant-time` + **`generation` = 会话内 turn 序号**）且**组件已实现**（`app/tool_execution/token_binding.py`，段二-4 已接线），**但没有任何调用方**。**实测确认（2026-09-13）**：① 无**工具调用分派**端点（`tool_call_id` 仅是消息 / 运行记录的字段）；② 无 `/export`、`/artifacts` 之类回调路由；③ 规格 **§2.3 已排除 dsh 发起反向请求**（只有 `initialize` / `session.prompt` / `shutdown` + 4 通知，**无审批方法**）⇒ **「请求授权」不可能由 dsh 发起**。**原定落点（"挂段二-4 的对话入口"）不成立** —— 那是**用户入口**，调用方**不是"执行侧"**。**⇒ 结论：无可挂的调用方，故未挂载**（避免造出"没人调用的校验"—— `/__revoke` 的前例）。**待办与解锁条件**：唯有**造出真实回调面**（例如让「导出产物」走**容器 → 工作台的 HTTP 回调**、随行短期令牌）才有落点；**但该做法会改变 §3.1.1 注 2 的既有落点**（`artifact.export` 现为"我方自建工具、**不经容器**"）⇒ **属地基级改动，须先出范围说明再动**。**在挂载并取证之前，不得声称「②④ 已强制」**（判据见 §5 **用例 35**）。 **〔2026-09-14 实现（方案 b-1）〕** **真实回调面 ＝ 执行回调边车**（`app/exec_callback`：`POST /internal/exec-callback`）——**独立进程、只监听一个端口、只暴露一个端点、只挂 `workbench-exec-internal`**（不双宿工作台默认网络；工作台 app 容器**不得**接入该内网）。**②④ 判定在边车内完成**：`expected` 从**服务端权威**的 `ActiveExecutionRegistry`（会话当前代次）重建、**绝不取自请求体**，与令牌自持绑定做 `constant-time` 比对；不匹配 / 跨租户 / 旧代次 / 未知令牌 → `403`（不泄露存在性）+ 记审计（复用 `tool.blocked`，**未新增动作码**）；放行后经 **`边车 → 工作台`** 的**受控出向调用**交回工作台。**取证**：`tests/test_exec_callback.py`（用例 35 六条**真端点端到端** + 暴露面「单端口 / 单路由」断言 + 反假 ≥3 组）、`tests/test_exec_callback_network.py`（**真容器**：执行容器**可达边车、不可达工作台**，补上此前空缺的断言）；网络顺序处置 = `scripts/ensure_exec_internal_network.py`（预创建内网桥）。 **〔2026-09-14 返工（§8 U21 裁决「候选②：边车纯转发 + 判定回工作台」）〕** 上述「判定在边车内 / 边车只挂执行内网」的落点**已被返工**：边车改为**无状态纯转发**、**双宿**执行内网 + 回调网，**②④ 判定回到工作台**（`POST /api/v1/internal/exec-callback`）；**详见 §8 U21**。 **残余未验证（不得读成已验；2026-09-14 §8 U21 返工后逐项更新）**：① **已收口（§8 U21）**：`边车 → 工作台` 链路现为**预共享密钥**鉴权（`WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`，恒时比对、**缺失即拒**）+ **`100` rps 限流**（超限 `429`），落点在**工作台接收端点**（边车已无状态）；**注**：限流为**进程内**口径（多副本 = 每副本 100 rps，**非全局**，未验证）；② **已由架构消解（§8 U21）**：判定回到工作台 ⇒ **无需跨进程共享状态**；权威状态（`TokenBindingStore` / `ActiveExecutionRegistry`）均为**工作台进程内**对象（**注**：工作台**多副本**时登记方与判定方可能落在不同副本，仍属进程内、**未验证**）；③ **`generation` 在真实多 turn 下的单调性未实测**（仅单测覆盖登记表单调性）；④ **边车多副本并发未验证**；⑤ **可达路径的落点已收口（§8 U21）** —— 新增**回调网**（`workbench-exec-callback`）作为 `边车 → 工作台` 的受控路径，编排 `docker-compose.exec-callback.yml` 已更新（边车双宿、工作台接入回调网）；**但该路径在真实容器下于本机（Windows / Docker Desktop）仍未实跑**（见 §8 U21 未验证项）；⑥ 本次未跑**真实 dsh turn 触发的回调**（回调面所需的「导出产物 / 请求授权」真实触发方仍未接入 —— 只做「端点到验证器到转发」的端到端，不经容器内的真实产物导出）。

### U21 跨进程权威同步 — ✅ **已闭环（2026-09-14 实现）**，下方登记未验证项

**原登记（2026-09-14 新增）**：❌ 未闭环 —— 从 U20 残余② 单列。

**缺口**：②④ 的权威数据（令牌自持绑定 `TokenBindingStore` 与「当前执行」登记表 `ActiveExecutionRegistry`）**都是进程内对象**；而生产部署下**签发令牌的是工作台进程、做校验的是回调边车（另一进程）** ⇒ 两进程**不共享**这些状态 ⇒ **边车独立启动时会把全部回调请求拒绝**（方向是 **fail-closed、不会误放行**，但**功能不通**）。**⇒ 后果**：②④ 的**机制、端点、判定、取证都已就位**，但**"生产拓扑下真正跑通"缺这一环** ⇒ **在此之前，不得声称「②④ 已强制」**（与 §U20 同一禁令）。**裁决（2026-09-14，用户）= 候选②「边车纯转发 + 判定回工作台」**：
  - 🔴 **限流口径裁决（2026-09-14，用户）＝ 保持「每副本 `100` rps」，不实现全局口径**：`100` rps 为**每副本**语义（**进程内令牌桶**，容量 `100` ≈ 1 秒额度，超限 `429`）。**⇒ 明确声明：全局（跨副本合计）`100` rps 未实现** —— 多副本部署下实际总吞吐 ≈ **副本数 × 100 rps**。**因此不得把该配置值读作「系统级上限」**；若要全局口径，须引入**分布式限流**（Redis 令牌桶等），**属未做项**。判据：现有用例只断言**单副本**的 `100` rps 速率与 `429`；**全局口径无任何取证**。
  - **⚠️ 先更正本条目此前的错误判断**：本条初稿曾写"候选②会与「执行容器不可达工作台」**冲突**" —— **这是错的**。中转由**边车**发起（出向），**执行容器仍不可达工作台**；候选②不但不冲突，**它恰恰最符合 §3.5 P1-3 的原裁决**（校验点 = **工作台控制面**）。
  - **收口方案**：**边车改为无状态纯转发**（内部不再持有 `TokenBindingStore` / `ActiveExecutionRegistry`）；**②④ 判定回到工作台**；新增一条**专用回调网**（只含「工作台 + 边车」）—— 执行容器只挂 `workbench-exec-internal`（**可达边车、不可达工作台**），**边车双宿**两网做受控转发，工作台只挂回调网。
  - **⇒ U21 由此消解**（**无需跨进程共享状态**）；并**顺带收口** U20 残余①（边车 → 工作台 的**鉴权与限流**）与残余⑤（**边车 → 工作台 可达路径**——回调网即该路径）。
  - **约束放宽（本次一并调整）**：U20 实现期写下的"边车**只挂 internal 网**"**放宽为「边车双宿 `workbench-exec-internal` + 回调网」** —— 原约束的**目的**是"不把工作台暴露给执行容器"，而**双宿不违反该目的**（边车仍**只监听一个端口、只暴露一个端点、只做转发**）。
  - **不得削弱**：`tests/test_exec_callback_network.py` 的「**执行容器不可达工作台**」断言**必须保持**，且须**新增**「执行容器**不可达回调网**」类断言以覆盖新网。**在收口并取证之前，仍不得声称「②④ 已强制」。**
  - **判据**：在**真实生产拓扑（两进程）**下，一次完整回调的**成功路径 + 拒绝路径**均需原始证据；且 **`tests/test_exec_callback_network.py` 的"执行容器不可达工作台"断言不得被削弱**。**同批提示**：U20 残余①（**边车 → 工作台 的鉴权与限流**）与本条**同批收口**为宜（同一条链路的两个缺口）。
  - **〔2026-09-14 落地（本轮返工）〕** **已实现**：① 边车**去状态** —— 移除 `TokenBindingStore` / `ActiveExecutionRegistry` 的使用（`app/exec_callback/registry.py` 删除，登记表迁至 `app/tool_execution/active_execution.py`），边车退化为**无状态纯转发**（源码内**无**任何绑定比对原语，AST 断言守护）；② **②④ 判定回到工作台** —— 新端点 `POST /api/v1/internal/exec-callback`（`app/tool_execution/callback_guard.py`）：`expected` 从**工作台权威状态**重建、`constant-time` 比对、不匹配 / 跨租户 / 旧代次 / 未知令牌 → `403`（不泄露存在性）+ 记审计（**复用 `tool.blocked`，未新增动作码**）；③ **专用回调网** `workbench-exec-callback`（只含「工作台 + 边车」）：**边车双宿**、**执行容器不可达回调网**（新增断言）；④ **鉴权 = 预共享密钥**（`WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`，恒时比对、**缺失即拒**）、**限流 = `100` rps**（超限 `429`）。**取证**：`tests/test_exec_callback.py`（工作台六条 + 边车纯转发 + 单进程端到端 + **反假 ≥3 组**）、`tests/test_exec_callback_network.py`（真容器：可达边车、不可达工作台、**不可达回调网**）。**未验证（不得读成已验；2026-09-14 实跑后逐项更新）**：
  - ① **两进程真实拓扑 —— ✅ 已跑通（两层，均有原始证据）**：**(层 1) 本机两进程 + 回环** —— 真工作台进程 + 真边车进程（**两个独立 PID**），成功路径 `200`；两条强证据：**客户端不带 PSK 仍得 `200`** ⇒ 密钥由**边车**补齐；**停掉边车进程 → `WinError 10061`（连接被积极拒绝）而工作台 `/api/v1/health` 仍 `200`** ⇒ **请求确经边车这个独立进程**（堵死"同进程两监听"造假）；**(层 2) 真容器 + 回调网（三容器）** —— **执行容器 → 边车容器 → 工作台容器** 容器间链路 `200`（工作台日志来源 IP ＝ 边车在回调网上的地址 `172.23.0.3`）；**隔离实测** `OK sidecar_ip/name` + `BLOCKED workbench_cb_ip` + `BLOCKED workbench_name` + `BLOCKED sidecar_callback_ip` + `BLOCKED internet` + `getent hosts wb → rc=2` + **无默认路由**；**拒绝路径** `403`（PSK 不一致）/ `403`（令牌不匹配，**文案与未知令牌完全一致**）/ `429`；**反假 4 组**（停边车→名称解析失败、**停工作台→`502 {"error":"upstream unreachable"}`**、改坏 PSK→`403`、路由/解析证据）。**两轮实跑均零仓库改动、临时 harness 与容器全清。**
  - ② **`generation` 真实多 turn** —— 仅单测覆盖登记表单调性与既有代次，**未经真实 dsh 多 turn 驱动**（**未验证**）。
  - ③ **限流口径 —— ✅ 已裁决**（见上方「限流口径裁决」条）：保持**每副本** `100` rps、**全局口径未实现**（多副本总吞吐 ≈ 副本数 × 100 rps）；**不得读作系统级上限**。
  - ④ **真实 dsh turn 触发的回调** —— **触发方仍不存在**（**未验证**）。
  - ⑤ 🔴 **实跑暴露的架构事实（已在真链路复现；非缺陷，但会误导人）**：**边车会用「自己的 PSK」覆盖客户端传入的 `X-Exec-Callback-Key`** ⇒ **「客户端故意传错 PSK」仍返回 `200`**；**「PSK 错误 → `403`」只能在「边车侧 PSK 与工作台不一致」时复现**。**⇒ 含义：有鉴权的是「边车 → 工作台」这一跳；「执行容器 → 边车」这一跳本身无鉴权**，它靠**短期令牌（在工作台端做 ②④ 判定）+ 网络隔离**保护（覆盖方向为 fail-safe，故**非漏洞**）。**建议硬化（未做）**：边车应**显式拒绝/剥除**客户端携带的该头，而非**静默覆盖** —— 属接口语义清晰化，**无安全影响**。
  - ⑥ **其它未验证**：生产 env（**postgres 持久化**）下的工作台容器；**多副本**下进程内限流与权威状态共享；**边车 `ThreadingHTTPServer` 的 `listen` backlog 默认 `5`** 导致高并发下 `ConnectionReset`、吞吐上限（常态约 50–65 rps，**未做稳定性评估**）；**`403` 的审计落库**（实跑为 memory 后端）；令牌吊销同步与 `generation>1`；**容器 → 宿主路径**（实跑**刻意不依赖**）。

### U22 密钥轮换的**触发方式**与**旧密钥配置落点** — ✅ **已闭环（2026-09-14 实现）**，下方登记未验证项

**原登记（2026-09-14 新增）**：❌ 未闭环

**现状**：§3.4 约束 1 已定**双密钥窗口**（新密钥加密、旧密钥**仅用于解密**；**窗口长度 = 最大审批超时 + 清理周期**；窗口内**不得**因轮换导致解密失败），且**机制本轮已实现**（`app/tool_execution/body_cipher.py` 的 `BodyCipher(key, *, previous_keys=())` + `rotation_window_seconds(...)`，见 §8 U15 / §5 用例 32）。

**🔴 缺口（真源未定义，本轮按令未自造）**：① **轮换的触发方式** —— 规格通篇只有"双密钥窗口 / 新加密旧解密 / 窗口长度"三句，**未定义谁在何时触发轮换**（无时间戳、无开关、无运维流程口径）；② **旧密钥的来源与配置落点** —— §4 的配置项中**只有一把**（**原登记时 23 项；本次落地后为 24 项**，新增项即下方裁决的 `…PREVIOUS_KEYS`） `WORKBENCH_BODY_ENCRYPTION_KEY`，且 §3.4 R3 把其格式**定死为单值** base64（32 字节原始密钥）；**没有任何"上一把密钥"的配置名 / 格式 / 存放与销毁口径**。

**⇒ 后果**：**双密钥机制的"能"已具备，但生产上无人能把旧密钥交给进程** ⇒ 轮换**事实上无法执行**。**在补齐之前，不得声称"密钥轮换已就绪"。**

**裁决（2026-09-14，用户）＝ 触发方式「启动时读配置」；旧密钥入新配置项**：① 新增配置项 **`WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`**（多值）承载旧密钥，**进程启动时读入** ⇒ **轮换 = 改配置 + 重启**（**明确：无热轮换**）；② **窗口结束后的销毁口径 = 由运维从配置中移除 + 重启**（进程不再持有该旧密钥）；③ 须同步**三处台账**（`app/settings.py` / `.env.staging.example` / `tests/test_env_templates.py`）+ **§B15 计数**；配置**缺失或为空即不使用旧密钥**，且**解不开必须 fail-closed**（抛 `BodyCipherError`，**不得**静默返回空）。**判据**：一次性"轮换前加密 → 配置旧密钥 → **重启后**成功解密"的端到端证据；且**窗口外（配置已移除）旧密钥必须失效**。**仍待取证（未验证）**：**多副本 / 多进程的窗口一致性**、**真库下的轮换路径**。
  - **〔2026-09-14 真进程实跑验证〕** **"真进程重启 + 旧密钥经配置解密"：✅ 已通过** —— 三个**真实独立进程**（`subprocess.Popen` 启动，**PID 各不相同**：`24932` / `44832` / `60728`；各自**在启动时读环境变量**并经**真实装配路径** `app.bootstrap.build_tool_execution(...)` 取得 cipher）：**A**（旧密钥、`PREVIOUS_KEYS` 为空）加密并把密文写入文件后退出 → **B**（**新密钥** + `PREVIOUS_KEYS`=旧密钥）**解密成功**（`plaintext_equals_sentinel=True`）→ **C（反假）**（新密钥、`PREVIOUS_KEYS` 为空）**抛 `BodyCipherError`、退出码 3**。**⇒ 原"只做到『重新装配』的等价模拟"这一项，据此销掉**（本次是**真进程重启**，非等价模拟）；**fail-closed 亦成立**（C 抛错证明旧密钥确系"经配置生效"，而非随便可解）。**〔2026-09-14 追加实跑（两项，销掉上面两条未验证）〕** ① **`PREVIOUS_KEYS` 多值：✅ 通过** —— 真进程下 `PREVIOUS=K1,K2` 时 **K1 / K2 两份密文均 `DECRYPT_OK`**；**反假** `PREVIOUS=K1`（只放一把）时 **K1 可解、K2 抛 `BodyCipherError`（退出码 3）** ⇒ 多值**逐项生效**。**解析边界（真源未规定，仅记录现状）**：空 / 纯空白 / `,` / `,,` ⇒ **空列表**；**含非法项时 `parse_previous_body_keys` 本身不报错**，由 `BodyCipher.from_base64` 抛 `ToolExecutionConfigError` 拒绝启用。② **窗口长度语义：✅ 已核对** —— 公式与常量出处交叉核验：`MAX_APPROVAL_TIMEOUT_MINUTES = 10080`（`app/workforce/models.py:30`）＋ 清理周期默认 `60`（`app/settings.py:468`）⇒ **当前生效窗口 = 604860 秒 ≈ 168.02 小时**；**反假**（改任一入参 ⇒ 返回值按公式变化，非硬编码）。
  - 🔴 **重要澄清（以代码为准，2026-09-14）：`rotation_window_seconds` 在 `app/` 内没有任何调用方**（仅 `app/tool_execution/body_cipher.py:43` 的定义 + `:8` 的 docstring 引用）⇒ **该时间窗口是「运维口径」，不是「代码机制」** —— 它指导运维"何时可以移除旧密钥配置并重启"。**真正的时间失效是逐行 TTL**：`body_expires_at = requested_at + approval_timeout_minutes`（`app/tool_execution/service.py:471`）→ 由 `BodyCleanupTask` 按 `body_expires_at <= now` 清理（`app/tool_execution/store.py:372-390`），**与 `rotation_window_seconds` 无任何代码关联**。**⇒ 不得声称"代码在强制该窗口"**（同 `/__revoke` 的教训：**存在 ≠ 被调用**）。
  - **仍未验证**：**多副本 / 多进程的窗口一致性**、**真库下的轮换路径**（本次只做到**文件级**密文，未落 `body_ciphertext` 真行）、**生产旧密钥的来源与保管**（未定）。

**〔2026-09-14 落地（本轮实现，明确「无热轮换」）〕** 已闭环：① **配置项** `WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS` 已入 `app/settings.py`（`body_encryption_previous_keys: str = ""`，**默认空 = 不使用旧密钥**）；**多值格式 = 逗号分隔的 `base64`（每个 = 32 字节原始密钥）** —— 真源（§3.4 R3）只把**单值**主密钥定死，**未规定多值格式**，故取既有配置风格里最贴近的一种（与 `cors_allowed_origins` / `content_scrape_allowed_domains` 同为逗号分隔；且 **base64 字符集不含逗号**，分隔无歧义），解析器 = `app/settings.py::parse_previous_body_keys`（空 / 空白 / 仅分隔符 ⇒ 空列表）。② **启动时接线（唯一读入点）**：`app/bootstrap.py::build_tool_execution` 装配处 `BodyCipher.from_base64(settings.body_encryption_key, previous=parse_previous_body_keys(settings.body_encryption_previous_keys))` —— **进程启动时读入一次，运行期不重读**（**未实现任何热加载 / 运行期重载**，与裁决一致）。③ **三处台账同批同步**：`app/settings.py`（段二 **23 → 24 项**）、`.env.staging.example`（新增 `WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS=`，**留空占位、无真实值**）、`tests/test_env_templates.py` 的 `STAGE2_SETTINGS_FIELDS`（**23 → 24**，并把新项纳入「fail-closed 默认留空」清单）；规格 §4 与门禁 §B15 的项数同批改为 **24**。④ **测试**：`tests/test_tool_execution_bootstrap.py` 三用例 —— **轮换端到端**（旧密钥加密 →「新配置含新密钥 + 旧密钥」的新实例解密成功）、**窗口外失效**（新实例只含新密钥 → 解旧密文抛 `BodyCipherError`）、**fail-closed**（配置缺失 / 空 / 仅分隔符 ⇒ 与「只含新密钥」行为一致）；`tests/test_body_cipher.py` 两用例覆盖多值解析与「解析后可用」。⑤ **反假 2 组**（**真变红**，原始输出见 `docs/change-record.md`）：装配处**忽略 `previous_keys`** ⇒ 轮换端到端用例红；`decrypt` 失败**改为返回空** ⇒ 窗口外失效 / fail-closed 用例红。**窗口长度公式 `rotation_window_seconds` 本次未改动**（按裁决）。
**仍待取证（未验证，不得读成已验）**：① **是否真的跑了「进程重启」** —— 本实现以「**重新调用装配函数构造新实例**」**等价模拟**「重启后读入配置」，**并非真实进程重启**（未起两个进程、未做滚动重启）；② **多副本 / 多进程的窗口一致性** —— 密钥集为**进程内对象**，多副本各自读配置，**未验证**；③ **真库下的轮换路径**（`workbench_tool_actions.body_ciphertext` 真行 + 真解密）**未验证**；④ **生产密钥的来源与保管**（旧密钥从哪来、存哪、谁有权读、如何注入）**未验证 / 未定**；⑤ 旧密钥**多值**仅做了「解析 + 解密可用」用例，**未验证「同时保留多把旧密钥」的运维流程**。
...
**⇒ 性质**：这是**同一"丙案机制"（J7）下的另一处漏点** —— 而 §3.4 约束 3 列举的四类持久层**不含消息表**，故该冲突**未在任何"落地清单"里被覆盖**。**在处置之前，不得声称"用例 32② 已全绿"**（其 (c1)③ 分支**判据与实现直接冲突**）。
...
| **N3** | ✅ `RuntimeRegistry.health()`（`app\runtime\registry.py:64-90`，原关键行 `:81` → 现行 `:86`） | **原事实**：`status` 只对 `ragflow` 特判 `unavailable`，其余一律强制 `"ok"` ⇒ **dsh 即使 `real_execution_enabled=False`（自身 `status="unavailable"`）也显示 ok** ⇒ **运维面板会误判** | 🔴 **具体缺陷** → ✅ **已修（2026-09-14，用户授权）** | **修法 = `status` 按 adapter 自报输出**：`raw["status"]` 为**字符串即原样透传**（不再按 key 特判）；**未上报 / 类型不符 ⇒ 维持既有默认 `ok`**。**依据**：① `health()` 是 adapter 的**自报契约**（`adapters/dsh.py:338-345` 自报 `ok`/`unavailable`、`adapters/ragflow.py:17-22` 恒报 `unavailable`）；② `status` 在**外部契约中为可选字段**（`docs/runtime-onboarding-request.md` §1.4）⇒ 缺省**沿用既有 `ok`**（`tests/test_runtime_api.py:46` 既已要求**不自报 status** 的 `mock` 显示 `ok`），**未自造新默认口径**；🔴 **若将来要求"缺省即 fail-closed"** ⇒ 属**真源未规定的口径**，**需另裁决**，本轮未做。**字段白名单口径未动**（仍为 `runtime/version/capabilities/sandbox/reason` + `status`）。**取证**：`tests/test_runtime_registry_config.py`（+2 用例：`dsh` 未启用真实执行 ⇒ `unavailable`（用**生产工厂** `build_dsh_adapter` 造真实 adapter 后 `register("dsh", …)`，非假 adapter）；未上报 status ⇒ 仍 `ok`）、`tests/test_runtime_adapters.py`（**原"非 ragflow 恒 ok"的固化断言已更正**为 `unavailable`）；**反假**（把 `status` 改回"非 ragflow 恒 ok" ⇒ 上述两用例**真变红**，原始输出见 `docs/change-record.md`）；**回归 `1708 passed, 31 skipped` + `compileall` exit=0**。⚠️ **不得延伸**：`dsh` **仍未注册进运行时**（见 **A3**）⇒ **不得说"运维面板已显示 dsh 的真实状态"**（面板里**根本没有 `dsh` 键**；本条只保证"注册表中若存在该 adapter，其自报状态会被如实输出"） |
| **N4** | `ExternalAdapter.health()`（`app\runtime\adapters\common.py:184-205`，原关键行 `:190` → 现行 `:194`） | **原事实**：`status` **写死为 `"ok"`**，**丢弃**上游 `GET {endpoint}/health` 自报的 `status`（仅透传 `version`/`capabilities`/`sandbox`/`reason`）⇒ **deerflow / codex_worker / hermes / agentscope 四个外部运行时**在下游自报不可用（如 `unavailable`）时**运维面板仍显示 ok**（**与 N3 同症、另一处**） | 🔴 **具体缺陷（同 N3 症）** → ✅ **已修（2026-09-14，用户授权）** | **修法 = `status` 透传上游自报**（**与 N3 同口径**，未自造）：`raw["status"]` 为**字符串即原样透传**；**未上报 / 类型不符 ⇒ 维持既有默认 `ok`**。**依据**：① `health()` 是 adapter 的**自报契约**；② `status` 在**外部契约中为可选字段**（`docs/runtime-onboarding-request.md` §1.4）⇒ 缺省**沿用既有 `ok`**（`tests/test_runtime_api.py:46` 既已要求不自报 status 的 `mock` 显示 `ok`）——**与 N3 缺省口径一致，未自造新默认**。**字段白名单 / 过滤语义未动**（仍为 `runtime/status` + 可选 `version/capabilities/sandbox/reason`）。**取证**：`tests/test_runtime_adapters.py`（**原把 N4 固化的断言**——`LeakyTransport` 自报 `unavailable` 却期望 `ok`——**已更正**为 `unavailable`；**新增正面验证** `test_external_health_passes_through_upstream_declared_status`：上游自报 `unavailable` ⇒ adapter 报 `unavailable`，上游未上报 ⇒ 仍 `ok`）；**反假**（把 `status` 改回写死 `"ok"` ⇒ 上述两用例**真变红**，原始输出见 `docs/change-record.md`）；**回归 `1709 passed, 31 skipped` + `compileall` exit=0**。⚠️ **不得延伸**：真实外部运行时（deerflow / codex_worker / hermes / agentscope）**未接入** ⇒ **不得说"运维面板已如实反映这些外部运行时的健康"**（未接入即面板无键） |
| **N5** | `CommercialLifecycleService.list_pending` → `execute_delete`（`app\commercial\lifecycle.py`） | **多副本 worker/beat 下，`list_pending` 取作业与 `execute_delete` 之间无锁**（取作业未加行锁、也非条件更新）⇒ 同一待执行删除作业可能被两个副本同时取到并各自执行 ⇒ **理论上存在重复执行窗口**（当前单副本默认部署下不可复现） | **(B) 预留 / 并发待收口**（非当前单副本缺陷；属多副本部署边界） | **登记（本轮登记项 ③，不修）**；**当前未测**（未起多副本 worker/beat，未构造并发窗口，**不得读成"已修/已防重"**）；**将来收口方向**：取作业时用**条件更新抢占**（`UPDATE workbench_lifecycle_jobs SET status='claimed' WHERE id=%s AND status IN ('cooling_down','queued') RETURNING …`，影响行数=0 即跳过），或 **`SELECT … FOR UPDATE SKIP LOCKED` 行锁**，使同一作业只被一个副本执行 |
| **R1** | `settings.object_storage_url`（`app\settings.py:15`） | `app/` 内**无读者**（仅 `scripts\staging_preflight.py` 按 env 直读）；`docs\key-dependency-autonomy-plan.md:165` 已自述"已声明、未接入" | (B) | **✅ 保留（2026-09-14，用户授权下逐面查证后判定）** —— **属「有文档记载的预留项」，非死代码**（**保留来路**：它**当前在 `app/` 内确无读者**，只是**删它与删死代码不是一回事**）。逐面依据：① **不在** `tests\test_env_templates.py:130-155` 的 `STAGE2_SETTINGS_FIELDS`（段二台账）内 ⇒ **无 §B15 计数 / 台账连带**；② **已登记**在部署配置面：`.env.example:12`、`.env.staging.example:19`、`docker-compose.app.yml:25`（`WORKBENCH_OBJECT_STORAGE_URL`）；③ **无任何测试**断言该 Settings 字段存在（`tests\test_staging_preflight.py:35,71` 用的是预检 **config dict 键**，非 Settings 字段）；④ `scripts\staging_preflight.py:72` 读的是 **env**（`os.getenv("WORKBENCH_OBJECT_STORAGE_URL", "")`），非 Settings 字段 ⇒ 删字段不连带预检脚本，但 env 名与字段名同源（`env_prefix="WORKBENCH_"`）；⑤ 文档将其记为**在用基础设施 + 将来接入对象**：`capability-ownership-map.md:52`（对象存储 = SeaweedFS ✅在用）、`architecture.md:14`、`api-contract.md:693-694` 与 dsh 规格 `:135/:272/:282`（`artifact.export` 落对象存储）、`key-dependency-autonomy-plan.md:617`（明言改该变量名会牵动 app 配置/preflight/测试/文档）。⇒ **删除会造成「模板 / compose 仍设该 env、应用侧却无对应字段」的漂移**，故**保留**（**未删**）。 |
| **R2** | `CommandGate.evaluate`（**原** `app\tool_execution\blacklist.py:320`） | ~~`app/` 内**无调用**（生产走 `verify_source` / `verify_name` / `verify_blacklist` 三步）~~ | (B) | **✅ 已删除（2026-09-14，用户授权）** —— 曾为死代码。三重检索：① **符号**：`CommandGate.evaluate` **全仓仅定义处命中**；生产 `service.py:381/386/391` 只走三步 `verify_source` / `verify_name` / `verify_blacklist`，**无任何 `.evaluate(`**；② **字符串 / 动态**：`["']evaluate["']` **0 命中**，`app/tool_execution/` 内 `getattr`（7 处）**均不指向 `evaluate`**；③ **测试**：`tests/` 内 `evaluate*` 全属 `evaluate_login_throttle` / `evaluate_task_idempotency` / `evaluate_plan_approval` / `evaluate_isolation*` 等他模块符号，**无 `CommandGate.evaluate`**；④ **反证（删前）**：定义改名（`evaluate_r2_rename_probe`）跑全量仍 `1709 passed, 31 skipped`（测试不引用）；⑤ **删后**：同命令仍 `1709 passed, 31 skipped` + `compileall` exit=0；**无失效 import**（`Sequence` / `Iterable` / `os` / `re` 均仍被其他方法使用）。**未改** `migrations/*`、**未新增**审计动作码、**未放宽**任何断言。 |

**⇒ 禁令延续**：涉及上述符号的结论**一律不得表述为"已生效"** —— N1 不得说"生产走 CLI 口径"；N2 不得说"支持注入 spec"；N3 **已修（注册表按 adapter 自报输出，2026-09-14）⇒ 可表述为"注册表中已有 adapter 的自报 `status` 会被如实输出"**，但 **`dsh` 未注册（A3）⇒ 不得说"健康面板反映 dsh 的真实状态"**（面板里没有 `dsh` 键）；N4 **已修（外部 adapter 按上游自报输出，2026-09-14）⇒ 可表述为"外部运行时若自报 `status` 会被如实输出"**，但**真实外部运行时（deerflow / codex_worker / hermes / agentscope）未接入 ⇒ 不得说"健康面板已如实反映外部运行时的健康"**（面板里没有这些键）；N5 **本轮仅登记、未修 ⇒ 不得说"删除作业已防重 / 已加锁"**（多副本重复执行窗口仍在，见 N5 行）。
...
- **E5 整模块 `app/commercial/plan.py`（`QuotaService` / `PlanVersion` / 套餐版本化 / 超额策略）** —— ✅ **已裁决：不属本期 ⇒ 预留（2026-09-14，用户裁决）**｜依据：与 `docs/superpowers/specs/2026-09-12-conversational-agent-platform-design.md:190` 决策 D1（「不在本期实现计费、白标、配额售卖」）取齐，并据此消解与 `docs/delivery-gates.md:58`（原已勾「套餐」）、`docs/capability-ownership-map.md:55`（「🟡 部分交付（G0）」）的口径冲突。
  - **保留**：`app/commercial/plan.py` 与表 `workbench_plan_versions`（`migrations/006_commercial_g0.sql:25`）**均不删**（避免「表在、代码没了」漂移）；**不得**声称「配额已强制」「套餐额度已生效」。
  - **⚠️ 明确不达**：G0 验收第 4 条（`docs/superpowers/specs/2026-09-06-commercial-g0-design.md:172`「超出套餐额度时按租户策略阻断、降级或进入审批」）**本期不达**；无配额端点、无前端诉求（`docs/api-contract.md` 商业化章节亦无套餐/配额条目）。
  - **将来接线时须先做**：确认 `plan.py` 与 `workbench_plan_versions` 同批启用，并补齐执行链路的额度检查点（真源未指定落点）。
...
### U27 E1–E4 实施暴露的「真源未定义 ⇒ 未实施」清单 — ✅ 已裁决（2026-09-14）

**性质**：下列 5 处缺口，是 **E1–E4（2026-09-14）实施过程中「因真源未定义而停下、未实施」**的部分（依宪法「没真源不开工」）。原登记目的 = 防止它们在后续被误读为「已实现 / 已生效」。**2026-09-14 用户已逐条裁决（见下表「当前状态」列）；原缺口描述与证据保留，作为「曾未定义」的来路。**

| # | 缺口 | 证据（文件:行，逐字） | 当前状态 | 需裁决什么 |
|---|---|---|---|---|
| 1 | **计量/定价口径未定义（E1 只能做方案 A）** | ① `docs\superpowers\specs\2026-09-12-usage-billing-page-design.md:49`「**不解释 `units` 的业务含义**：账本里没有单位语义，页面只如实展示原值，不猜测是 token / 次数 / 条数。」；② 同文件 `:66`「2. **`units` 语义未定义**：只展示原值，不标注单位。」；③ `docs\superpowers\specs\2026-09-06-commercial-g0-design.md:96`「客户端不能提交自己的预算或计费结果；服务端根据**租户套餐、岗位预算和任务快照**计算」—— 而 **E5 已裁「不属本期」**（`app/commercial/plan.py` 整模块预留，见 `docs\change-record.md` 2026-09-14 条）⇒ **该计算的依据不存在**；④ `app\settings.py` **无任何计价配置**（检索 `cost` / `price` / `pricing` / `计量` / `计价` / `billing` / `cents` 等 **0 命中**）。 | **✅ 已裁决（2026-09-14）**：**维持现状** —— `units=1`（1 run = 1 unit）、`cost_cents` **恒 0**（与真源「内部版不向员工计费」一致）。实现留痕：运行终态记 `units=1`、`cost_cents=0`（`app\runtime\service.py` 的 `_record_usage_on_terminal`，现行 `:355`，`units=1` 于 `:383` / `cost_cents=0` 于 `:384`；commit `f719e60`）；`units` 语义**沿用「未定义、只展示原值」**。 | `units` 的业务语义与 `cost_cents` 的单价来源（若需定价 ⇒ 属**新增真源**，牵动 `migrations` 与三处台账）。 |
| 2 | **导出包过期时长未定义** | `docs\superpowers\specs\2026-09-06-commercial-g0-design.md:110`「导出任务异步执行，生成**带过期时间**的下载包」—— **未给时长**；全仓检索**无取值**。 | **✅ 已裁决（2026-09-14）**：过期时长 = **7 天**（`expires_at = created_at + 7 天`）。实现留痕：`app\commercial\lifecycle.py` 新增 `EXPORT_PACKAGE_TTL = timedelta(days=7)`，worker 导出作业落库时以 `now + EXPORT_PACKAGE_TTL` 显式传入 `store_export_package`。原缺口描述（**曾未定义 ⇒ 未实施**）：迁移 `028` 已建 `workbench_export_packages.expires_at TIMESTAMPTZ NOT NULL`（`migrations\028_workbench_export_packages.sql:15`，**真库演练已验证 DDL 与强制力**）；但 `expires_at` 是**显式入参**（`app\commercial\lifecycle.py:71` `ExportPackage.expires_at`、`:210-217` `store_export_package(*, expires_at)`），此前**生产装配下没有任何调用方传入**（`store_export_package` 全仓**仅测试命中** `tests\test_commercial_lifecycle.py:117`）⇒ 与 U25 同族的「存在但无调用方」。 | 过期时长取值。**⇒ ✅ 已裁决（2026-09-14）= 7 天。** |
| 3 | **导出载荷数据面未实现（15 类全为空数组）** | 真源类别清单 `docs\superpowers\specs\2026-09-06-commercial-g0-design.md:100-110`（用户、岗位、数字员工配置；任务、运行、步骤、产物元数据；知识文档元数据、版本和引用关系；记忆、成长提案和审核记录；用量账本和审计记录）；脱敏契约 `docs\api-contract.md:142-144`「导出内容经过脱敏，不包含密码、Cookie、验证码、令牌、原始 API 密钥或客户原文」；G0 验收 5（同规格 `:173`「5. 客户管理员可以申请并下载脱敏导出包。」）。 | **✅ 已裁决（2026-09-14）**：数据面**本期不建** —— 15 类保持空数组 + 沿用 `UNIMPLEMENTED_EXPORT_CATEGORIES` 登记。实现留痕：`app\commercial\lifecycle.py` 的 `build_export_payload`（`:193`）产出**真结构**（真源 15 类）、随机密约定，但**商业化服务不持有这些数据的读取通道** ⇒ 15 类**全部为空数组**，已显式登记于 `UNIMPLEMENTED_EXPORT_CATEGORIES`（`lifecycle.py:43`；commit `a597d8e`）。 | 是否本期建数据面。**⇒ ✅ 已裁决（2026-09-14）= 本期不建。** |
| 4 | **「最终导出」的判定规则未定义（E3 前置）** | `docs\superpowers\specs\2026-09-06-commercial-g0-design.md:114`「租户删除采用两步流程：管理员申请 → 冷静期 → 执行删除。**删除前必须生成最终导出包并记录确认人**」与 `docs\api-contract.md:148`「冷静期结束前必须完成最终导出」**均未定义「哪次导出算最终」**；且 `app\commercial\lifecycle.py:50-58` 的 `LifecycleJob` **无「导出作业 ↔ 删除作业」关联**（字段仅 `tenant_id` / `kind` / `status` / `requested_by` / `execute_after` / `id` / `requested_at` / `final_exported`）。 | **✅ 已裁决（2026-09-14）**：判定规则 = **删除申请之后首次完成的导出即「最终导出」**（据此置 `final_exported` 并与删除作业关联）。实现留痕：worker 完成导出作业后，若该租户存在处于冷静期（`cooling_down`）且尚未 `final_exported` 的删除作业，即调用 `mark_final_exported` 置位（**用既有字段判定「首次」，不新增列**）。原缺口描述（**曾未定义 ⇒ 未实施**）：`app\commercial\lifecycle.py:50-58` 的 `LifecycleJob` 无「导出作业 ↔ 删除作业」关联；`mark_final_exported`（`app\commercial\lifecycle.py:245`）此前**仍未接线**（E3 未实施；全仓**仅测试命中** `tests\test_commercial_lifecycle.py:48`，生产无调用方）。 | 判定规则（建议候选：**删除申请后首次完成的导出即「最终」**；或由管理员显式标记）。**⇒ ✅ 已裁决（2026-09-14）= 删除申请后首次完成的导出。** |
