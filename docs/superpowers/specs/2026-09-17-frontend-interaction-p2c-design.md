...
>
> **交付记录（P2c-5，2026-09-17 本机）**：① **实施裁定先行**（已写入 §2.12）——§5 裁决 12 落地为**复制最小实现**（不做 workspace 改造，避免触碰已交付并稳定的 admin-web 构建面）；**明示简化范围**：无 run 解析与钉死（始终尾随该会话最新 run）/ 无右舞台 / 折叠条只渲染帧标签 / 不并发预开流（按发送响应的 `X-Stream-Run-Id` 开流，帧从 `seq 1` 补发不丢帧）。② **手机伴侣端（全新对话面板，此前无对话能力）**：新增 `features/conversation/*`——`types.ts`（类型 / 分页常量 / 帧标签）/ `stream.ts`（`eventsource-parser@4.1.1` 帧解析，最小复制）/ `invocation.ts`（结构化调用判定，发送路径路由同 §2.4）/ `api.ts`（`Bearer` + 状态码映射 + `ConversationApiError` / `ConversationSessionExpiredError`）/ `useConversations`（列表分页）/ `useConversationDetail`（详情与消息窗口）/ `useConversationStream`（`fetch` 读流 + `Last-Event-ID` 真增量续播 + 指数退避 + 终态关流 + 无 run 关流如实告知 + 切走 `abort` + ≤100ms 合批）/ `ProcessBar.tsx`（简化折叠条，≤500 行窗口）/ `ConversationPanel.tsx`（列表 → 详情两级 + 消息 + 发送 + 载入/空/错误/无权限四态）；`App.tsx` 增「待办 / 对话」标签（**非激活标签不挂载** ⇒ 轮询与读端随切换停止，既有审批与收件箱组件与测试**零改动**）；`styles/app.css` 补面板样式。③ **桌面端核对**：新增 `desktop/scripts/desktop-stream-check.cjs`（在**真实 Electron 进程**内：本地 SSE 服务器造两段带间隔数据核对**增量**流式读 + CSP 全链路取证 + 后端列表与读端连通性；**不参与** **`node --test`**，不进入 CI）。
> **验证**：`companion-pwa` **56 passed（10 文件）** + `npm run build` 通过（新增 19 条：解析 4 / 读端 6 / 面板 9，覆盖判据「登录态 / 列表 / 发送 / 简化流 / 错误四态」）；`admin-web` **261 passed（36 文件）** + `tsc -b && vite build`（零破坏复核）；`desktop` **19 passed**（未误抓核对脚本）；后端 **2395 passed** + `py -m compileall -q app tests` exit 0（本批**零后端改动**）；`npm ci --dry-run` 通过（lockfile 与 `package.json` 同步）。**反假两项已实测变红（均已复原）**：① 读端重连起点固定 `0` ⇒「续播」用例红；② 纯文本也塞进结构化路径 ⇒「发送路由」用例红。**桌面端核对（实测取证，退出码 0）**：Electron `44.3.0` / Chromium `152.0.7977.78`；**远程模式**（`http://localhost:5173/`）与**内置模式**（`file://` 加载 `desktop/web` 产物）**双双实测两段增量到达**（间隔 306ms / 306ms，非整体缓冲）；CSP 全链路为空（页面 meta / 导航响应头 / 本地 SSE / 后端列表**均无 CSP**）⇒ 不存在 CSP 拦截；后端列表 200、SSE 读端 200 `text/event-stream`、**无 run 时无** **`X-Stream-Run-Id`** **且连接保持**（2.5s 内无帧，符合「等待 run」语义）。`desktop/web` 由 `npm run copy:web` 生成（已 gitignore，未入库）。
...
> **CI 销账（2026-09-18，北京时间跨零点推送）**：提交 `cb3a80f` 推送后（`e6f1d35..cb3a80f main -> main`）run **`35245229532`** **⇒ 六 job 全绿**（后端 pytest + compileall、后端真库（Postgres service + `*_postgres.py`，含新迁移 `039` 与 `test_conversation_members_postgres.py`）、**沙箱加固与真容器回归**、网页管理台 vitest + build、手机伴侣端 vitest + build、桌面端 node --test）。
> **收尾补记（2026-09-18）**：**契约回改核对 14 处**（含 1 处真不一致：导出响应字段枚举补 `sender_id`）+ **参与者列表补分页（收尾裁决 B：`limit`** **1–200 默认 200 /** **`offset`** **≥0，响应只增** **`limit`/`offset`，`total`** **恒为命中总数）**——提交 `d9feae1` 推送后 run **`35247614898`** **⇒ 六 job 全绿**；后端 2418 passed / admin-web 267（37 文件）/ PWA 57 / desktop 19，反假两轮变红复原。明细见 change-record 同日条目。
...
| <br />    | B | **对话页三区**                            | 会话列 + 对话流（消息 + 过程折叠条）+ 右侧舞台（运行概览 / 过程时间线 / 审批）；窄屏舞台折叠为抽屉                                                                  | 前端      |
| <br />    | C | **流消费（SSE 读端）**                      | `fetch` + 读流；`Last-Event-ID` 真增量续播；心跳忽略；终态关流；退避重连；切换会话 / 离开视图即 `abort`                                                    | 前端      |
| <br />    | D | **发送路径路由**                           | 结构化调用 ⇒ 带键 `messages:stream`（真实执行 + 开流）；纯文本 ⇒ 不带键（桩、无流）——修复「纯文本必 422」不一致                                                  | 前端      |
| <br />    | E | **内联审批卡 + 收尾检查（展示型）**                | 审批在对话内决议（复用既有接口、权威态回流）；运行收尾展示「进度档」（未达标记「未完成」，**不改状态、不退回**）                                                                | 前端      |
| <br />    | F | **性能与状态纪律**                          | 流状态隔离 + 帧 ≤100ms 合批 + 时间线窗口（≤500 行）+ 跟随阈值；状态词沿用既有标签，未知 kind/字段不猜测                                                         | 前端      |
| **P2c-2** | G | **契约修订：内容级回传（有界）**                   | `cmd.run` 输出与文件变更以**有界摘录**回传（默认 16 KiB/次、二进制不回传、过 `redact_payload`、随帧 7 天保留）；**修订 Q9**（仍不落全文）                             | 后端 + 契约 |
| <br />    | H | **审批后推进的过程事件**                       | resume（决议后推进）路径接入帧写入 ⇒ 「决议后继续执行」也实时可见                                                                                     | 后端      |
| <br />    | I | **终端输出面板 + 文件 diff 面板**              | 右舞台新增两个面板（数据 = G 的回传），可展开、可截断告知                                                                                           | 前端      |
...
| <br />    | K | **产物登记 + 产出 chip**                   | 新表登记产物（run 级、跨执行可查、保留期）+ 只读端点；对话流内产出 chip → 展开 / 跳转                                                                       | 后端 + 前端 |
| **P2c-4** | L | **每会话模式（Ask / Plan / Goal / Craft）** | 新列 + 与自治三档**取更严**合成（`ask` 拒执行 / `plan` 强制待批 / `goal`·`craft` 不放松）；默认 `craft`＝现状                                           | 后端 + 前端 |
| <br />    | M | **模型 / 工具选择器**                       | 新增两个**只读**候选端点（`super_admin`）；「不可用项给原因」；**推翻契约 Y1**（留痕）                                                                   | 后端 + 前端 |
| <br />    | N | **个人数据导出 / 物理删除（合规）**                | 导出本人会话与消息（**库内实际存在的字段**）；删除 = **真删消息 / 帧 / 流状态 / 幂等行** + 会话软删与标题清空；运行与审计保留（不含正文）；复删幂等                                     | 后端 + 前端 |
| <br />    | Q | **自动验收 + 一键重做**                      | 收尾检查升级：**结构判定**（步骤全完成 + 无未决审批 + 正常终态）标「达标 / 未达标」；未达标给**一键重做**（页面持有原件时以新幂等键重发）；**不调模型、不自动重跑**；判定不改运行状态                     | 后端 + 前端 |
| **P2c-5** | O | **PWA 对话面板（含流）**                     | PWA **新建**对话页：列表 / 消息 / 发送 / 过程折叠条（简化版，无右舞台）；复用既有登录与审批页                                                                   | PWA     |
| <br />    | P | **桌面端核对**                            | Electron 外壳加载 admin-web ⇒ 继承；核对 `fetch` 流式读与 CSP（未验证项 §6）                                                                 | 核对      |
| **P2c-6** | R | **会话分享（本租户指定用户 · 只读）**               | 成员表（`read` / `write` 两档）+ 增删查端点；读路径统一「**本人 ∪ 成员**」；仅点名、仅本租户；撤销写审计                                                         | 后端 + 前端 |
| <br />    | S | **多端协同（受邀成员可发言）**                    | `write` 成员可在会话发言与**以其本人身份**触发执行（不放松任何既有闸门）；消息新增 `sender_id`；参与者列表 + 最近活动（**不做实时在线态**）                                     | 后端 + 前端 |
...
* **PWA**：新增对话面板（列表 / 消息 / 发送 / 过程折叠条简化版）；复用既有登录态与审批页；**不做**侧栏重组与右舞台；流消费复用同一读端实现（抽公共模块或复制最小实现，§5）。
  ...
  * **P2c-5 实施裁定（2026-09-17，§5 裁决 12 落地选择）**：采用**复制最小实现**——把读端最小子集（帧解析 / `Last-Event-ID` 真增量续播 / 指数退避 / 终态关流 / 切走 `abort`）复制进 `companion-pwa`。**不做 workspace 改造**：`admin-web` 与 `companion-pwa` 各自独立 `package-lock.json`，抽共享包会动已交付并稳定的 admin-web 构建面（违背「零破坏」优先）。**简化范围（明示）**：无 run 解析与钉死（始终尾随该会话最新 run，服务端自动发现）；无右舞台（无概览 / 审批 / 终端 / diff / 产物 / 结构判定）；折叠条只渲染帧标签（不渲染 payload 明细字段）。

  * **发送路由（沿用 §2.4 口径）**：结构化调用 ⇒ `messages:stream` + 新幂等键；纯文本 ⇒ 不带键（桩路径）。**未产生运行**（后端未装配真实执行）⇒ 关流 + 如实告知「本次没有过程流」，不留假过程条（同 §2.3 口径）。
...
### 2.17 【P2d · 2026-09-18 变更】会话主线进左栏 + 帮助体系（**本节修订 §2.1 / §2.2**）

> **性质**：**修订节**。§2.1（侧栏重组）与 §2.2（对话页三区）的原文**保留作历史**，凡与本节冲突处**以本节为准**。
> **触发**：批 1 审计 `ux-audit-batch1-information-architecture.md` 的 **IA-01 / IA-02 / IA-03**（P1，实测坐实"用户投诉的核心"）；批 4 审计 `ux-audit-batch4-copy-and-help.md` 的 **E-01～E-04**（P1/P2，全站零帮助入口 / 无首访引导 / 提示不成体系 / 无快捷键）。
> **判定**：现行实现**符合** §2.1/§2.2 原文（会话列确在对话页内），因此**不是实现缺陷，是设计要求变更** ⇒ 按铁律 3 先改本真源、再改代码。

#### 2.17.1 左栏改为三段式（**替代 §2.1 的"6 组平级列表"形态**）

| 段      | 内容                       | 纪律                                              |
| ------ | ------------------------ | ----------------------------------------------- |
| **顶部** | **「＋ 新建会话」主按钮**（常驻）      | **任意视图可点**；点击后直接进入可输入的新会话；键盘可达、`aria-label` 完整  |
| **中部** | **会话列表**（全部 / 进行中 / 已归档） | **常驻**，任意视图可见；点击某会话即切换，**不得丢失当前会话上下文**（修 IA-03） |
| **下部** | 6 组导航（**默认折叠**，只显组标题）    | 保留"**每条入口必须指向已交付页面**"；**记住展开状态**（跨会话）           |

#### 2.17.2 对话页收缩为两区（**替代 §2.2 的三区**）

| 区            | 内容                                          | 数据源              |
| ------------ | ------------------------------------------- | ---------------- |
| **① 对话流（主）** | 消息 + 过程折叠条（运行中当前步/计数；终态收为一行）                | 消息 = 会话详情；过程 = 帧 |
| **② 右侧舞台**   | 运行概览 / 过程时间线 / 工具流水 / 终端输出 / 文件改动 / 产物 / 审批 | 同 §2.2           |

* 原 §2.2 的 **① 会话列（左）** **移出对话页**，改为左栏中部（§2.17.1）。

* 窄屏（<1024px）舞台折叠为抽屉（默认收起）**不变**。

#### 2.17.3 会话列表的**单一可信来源**（数据纪律）

* 会话列表由 **`ConversationListProvider`（App 层）** 持有；**左栏与对话页读同一份状态**。

* 写操作（新建 / 归档 / 删除 / 成员变更引起的可见性变化）后由 **Provider 统一刷新**。

* **禁止**两处各自调用 `listConversations`（否则会出现"左栏新建了、页内列表不刷新"）。

#### 2.17.4 帮助体系（**新增**，对应 E-01～E-04）

| 件           | 行为                                                      | 对应                 |
| ----------- | ------------------------------------------------------- | ------------------ |
| **「?」提示按钮** | 关键区域标题旁；点开一句话解释 + 常见操作；`Esc` 关闭；`aria-label` 说明所释对象     | E-03               |
| **首访引导**    | **一次性**、可关闭、**按页记忆**；给三条典型路径（新建会话 / 看数字员工 / 处理通知）       | E-02               |
| **帮助面板**    | 侧栏底部「帮助与反馈」入口；含**快捷键表** + **版本号（关于）**                   | E-01 / E-04 / E-05 |
| **快捷键**     | `?` 打开帮助面板 · `Esc` 关闭弹层 · `Enter` 发送 · `Shift+Enter` 换行 | E-04               |

* **一律业务语言**：不出现"桩回复 / P1 / 数据模型 / 后端"等开发术语（对应 D-01）。

* 帮助内容**内置**（弹层 / 浮层），**不外链**（离线与内网环境可用）。

#### 2.17.5 文案修正（**D-01 收口**）

* 对话页面向用户的 6 处开发者文案改业务语言；**生产环境隐藏桩标识**，开发态可保留。

#### 2.17.6 边界（**明确不做**）

* **不动**后端 / API / 数据模型 / 权限模型。

* **不动** `?view=` URL 模型（保住深链 G-01 与会话深链 G-03）。

* **不动**配色与设计 Token（除新增组件取用既有语义变量）。

* 不引入路由库；不引入新运行时依赖（帮助弹层用现有 React + CSS 实现）。

#### 2.17.7 验收标准

| #  | 标准                                        | 验证方式              |
| -- | ----------------------------------------- | ----------------- |
| 1  | 任意视图下左栏首项为「＋ 新建会话」，点击进入可输入会话              | 实测 + 用例           |
| 2  | 任意视图下左栏可见会话列表；点击切换会话**不丢当前上下文**           | 实测 + 用例（URL 前后对比） |
| 3  | 会话列表为单一来源：左栏新建后，对话页列表**同步出现**该会话          | 用例                |
| 4  | 6 组导航默认折叠、展开状态跨会话保持                       | 用例                |
| 5  | 「?」按钮可开可关、`Esc` 可关、焦点不丢                   | 用例 + 键盘走查         |
| 6  | 首访引导仅首次出现；关闭后不再打扰                         | 用例                |
| 7  | 帮助面板含快捷键表与版本号                             | 用例                |
| 8  | 生产文案中不出现「桩回复 / P1 / 数据模型 / 后端」            | 构建产物检索            |
| 9  | 既有 admin-web 用例**全绿**（回归）                 | `npm test`        |
| 10 | **反假测试**：把会话列表数据源改为空 ⇒ 标准 2/3 的用例必须**变红** | 变异测试              |
| 11 | **截图 / 录屏取证**（本轮首次具备条件，替代批 1 的纯 DOM 证据）   | 人工走查              |

#### 2.17.8 验收结果（2026-09-18 · 逐条实测，**未验证项如实标注**）

| #  | 结果 | 证据                                                                                                                                                                            |
| -- | -- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1  | ✅  | 用例 `creates a conversation from the sidebar and opens it`；浏览器实测：点左栏「新建会话」⇒ URL 变为 `?view=conversation&conversation=conv-666caa373191`（新 id），输入框可用、占位文案「给数字员工发一条消息…」           |
| 2  | ✅  | 用例 `keeps the open conversation when the 对话 entry is clicked again (IA-03)`；浏览器实测：点侧栏会话项 ⇒ URL 变为 `…conversation=conv-32634192aa59`、主区标题随之切换                                  |
| 3  | ✅  | 用例 `loads the list exactly once through the provider`（断言该请求**恰好 1 次**）；浏览器实测：左栏新建后侧栏计数由「共 3 条」变「共 4 条」（同一份状态）                                                                 |
| 4  | ✅  | 用例 `collapses every navigation group by default and remembers the expansion`；浏览器实测：6 组 `aria-expanded="false"`、`.nav-item` 数量 **0**                                           |
| 5  | ✅  | 用例 `opens the help panel with ? and closes it with Escape…`；浏览器实测：`?` 打开、`Esc` 关闭后 `[role=dialog]` 数量 0                                                                       |
| 6  | ✅  | 用例 `shows the onboarding only on the first visit`；浏览器实测：首次进入出现、点「知道了」后消失且写入 `workbench.onboarding.seen`                                                                       |
| 7  | ✅  | 同上（快捷键 4 条 + 末行「版本 0.1.0」，版本号取自 `package.json` 单一来源）                                                                                                                          |
| 8  | ✅  | **生产构建产物机检**：`桩回复` / `P1` / `数据模型` **0 命中**（开发态文案改为内联字面量后，生产构建把该分支整体折叠）⇒ ⚠️ **边界**：`后端` 一词仍出现在**其它页面**（数字员工设置 / 运行详情 / 若干字段提示），**属批 4 的 D-02 / D-05 / D-06，不在 §2.17 范围、本批未修** |
| 9  | ✅  | admin-web 全量 **38 文件全通过**（原 37 → 新增 `src/app/SidebarConversations.test.tsx`；原「列表条目展示会话模式标签」用例随会话列迁入左栏，由新文件覆盖）· `tsc -b` rc=0 · `vite build` 成功                                |
| 10 | ✅  | **反假测试**：把 `setConversations` 改为恒空 ⇒ `SidebarConversations` **5 个用例变红**（错误分支用例保持绿）⇒ 用例真实有效；已还原并复测全绿                                                                           |
| 11 | ✅  | 截图 4 张：`.acceptance/ui-redesign/01-onboarding.png`（首访引导）· `02-sidebar.png`（左栏三段式）· `03-help.png`（帮助面板）· `04-new-conversation.png`（左栏新建后）                                      |

**取证环境偏差（如实登记）**：内置浏览器视口仅 **552×489**（触发 `max-width:720px` 断点、分组标题被响应式隐藏），故改用外部桌面浏览器 **1183×729**（截图 2071×1276）；**未达到 1440×900** 的原计划，亦**未做录屏**。**仍未验证**：窄屏（<1024px）舞台抽屉与左栏的叠加表现 · 真机 DPI / 多显示器 · 深浅色两套下的新组件外观。

**补记（2026-09-18 复核轮 · §156）**：自检时发现**窄屏（≤720px）下"默认折叠"会造成导航死路** —— 该断点的 CSS 本就隐藏组标题、靠"全部导航项横向滚动"兜住，而折叠后导航项**不在 DOM 里** ⇒ 窄屏**没有任何导航入口**。**已修**：`AppShell` 在窄屏**强制展开**（收纳交给横向滚动）+ 单测 + 反假测试。**窄屏真实浏览器呈现仍未人工验证**。
...
7. **PWA 对话面板：已交付**（P2c-5）——`companion-pwa` 新增 `features/conversation/*`（列表 / 消息 / 发送 / 过程折叠条简化版 + 最小复制读端）；**未取证项**：① 真实帧流在 PWA 的端到端（本机 dev 未装配真实执行 ⇒ 无运行、无帧；组件级用例以桩响应覆盖解析 / 续播 / 终态 / 无 run 关流）；② 真机安装性与移动端手势未测（承接既有 PWA 未验收项）。
