...
- ✅ **真实 WeKnora 实例联调（N2，2026-09-15，本机自建；非 staging、非 CI）**：按官方镜像 `wechatopenai/weknora-{app,docreader}:v0.8.0` + `paradedb/paradedb:v0.22.2-pg17` + `redis:7.0-alpine` 起 compose（app 8181，DB/Redis 仅容器内网），用 `config/builtin_models.yaml` 声明式注册**本机 bge-m3（1024 维）** 为 Embedding 内置模型，另起一个**本机 OpenAI 兼容 embedding 端点**（脚手架，仓库外）供其调用。**脚手架与联调环境均在仓库外**（`D:\徐徐AI学习\_weknora-verify\`；**2026-09-16 已按用户确认清理**：仅保留证据 `probe-evidence.json`，克隆目录 / 端点脚手架 / 探针脚本 / 日志 / 容器 / 卷 / 镜像均已删；详见 change-record N2 条影响面），未进仓库。
...
| # | 项 | 状态 |
| --- | --- | --- |
| N1 | **存量文档（无登记）** | ✅ **已落地（2026-09-15，用户确认本期做）**：脚本 `scripts/knowledge_import_register.py`——读**清单文件**（JSON / CSV）→ 逐条经服务层闸门登记为 `draft`（`source_key='migration'`、**owner 留空待人工补**）；**默认 dry-run**（不加 `--apply` 绝不写库）、逐行拒绝不静默丢弃、幂等（已登记一律 skipped）、审计 actor = `--actor-id`。✅ **缺口已收口（2026-09-16，用户指示「继续处理 N1 剩余缺口」）**：规格原文「读 WeKnora 文档列表」**已实现**——新增数据源 `--source weknora`，经 `WeKnoraKnowledgeAdapter.list_documents`（`GET /api/v1/knowledge-bases/{id}/knowledge`）逐页拉到 `total`；**接口面依据**：上游官方契约（`docs/api/knowledge.md`：查询参数 `page`/`page_size`/`parse_status`，响应 `{"data":[...], "page", "page_size", "total", "success"}`）+ **2026-09-15 真实实例实调该端点**（N2 联调期间用于复用已有文档，`data` 为数组已实证）。**安全口径**：翻页超 `--max-pages` **显式失败**（绝不只导入一部分）、上游 `success:false` fail-closed、条目缺 id 计入 rejected、API Key 只从参数/环境变量注入且**从不回显**（只打印上游主机名）。**未验证**：本机验证实例已于 2026-09-16 清理 ⇒ **该数据源未在真实实例上端到端复跑**（单测覆盖分页/上限/拒绝/凭据失败路径，均以 MockTransport 与假适配器完成）。真库回归见 §0（`test_import_register_script_against_postgres`）。 |
