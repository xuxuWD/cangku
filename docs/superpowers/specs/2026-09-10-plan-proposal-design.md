...
- `MockPlanGenerator`：按目标关键词与固定模板产出步骤，结果确定可重复，仅使用白名单工具。
- `ModelPlanGenerator`：通过 OpenAI 兼容接口请求结构化步骤；解析失败、超出步数上限、缺少必需字段、工具不在白名单时，**一律抛出明确错误，不静默降级为 Mock**。模型无权选择或声明模型，调用前必须经既有 `ModelGateway` 校验该模型获准处理当前数据分级，未获准则不发起调用。
...
- `propose(actor, task_id, goal, idempotency_key)`：校验任务归属与租户；按同一任务、同一幂等键返回既有提案；调用生成器；归一化步骤；写入 `pending_review` 提案与审计。
...
模型选择与连接配置是两件事：连接信息来自上面的 planner 专属配置（单模型直连，形态与内容工作台的 OpenAI 兼容后端一致）；**调用之前仍必须经既有 `ModelGateway` 校验该模型是否获准处理当前数据分级**，未获准时不发起调用并返回明确错误。这样既保留单模型部署的简单性，又不放弃数据分级这道闸门。

## 错误与一致性

- 未知工具：拒绝整个计划，返回 `422` 并指出违规工具名，不生成部分计划。
- 模型输出无法解析或超出步数上限：返回 `422`，任务与提案状态不进入可用态。
- 未审批即执行：返回 `409`。
...
- 新增 `migrations/010_plan_proposals.sql` 保存计划提案。
