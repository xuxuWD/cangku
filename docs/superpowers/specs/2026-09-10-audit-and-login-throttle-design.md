...
- 白名单只校验**顶层键名**，不递归校验值的内容。当前所有 `detail` 值都是服务端写入的标量（字符串、整数、布尔），因此不会触发该缺口；若后续需要在明细里写入嵌套结构，必须同时补上「对值递归检查敏感键」的校验与测试。**（已于 2026-09-11 补齐：`build_record` 对每个明细值调用 `redaction.has_sensitive_key` 递归校验，见 `docs/security-attack-surface-report.md` 第 4 节第 5 条。）**
