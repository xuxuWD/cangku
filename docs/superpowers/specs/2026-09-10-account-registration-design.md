...
以下三项由项目宪法明确要求，但本轮未实现，登记为遗留项，不视为本设计已完成，也不据此宣称生产可用：

- **登录限流缺失**（宪法第三章第一道防线「同一账号 5 分钟内错误超 5 次锁定」）：`POST /api/v1/auth/sessions` 可被无限尝试，且每次失败尝试都带有固定的 scrypt 成本。生产环境必须先在网关层补齐限流与失败锁定。
