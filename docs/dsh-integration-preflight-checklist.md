...
> **前提**：C2/C3 已实跑（见本文档 **§F9**），**「成功态」已于 2026-09-13 以桩上游取得（见 §F12）**。⚠️ **但「真实供应商下的 turn 成功」仍未取得**（凭据 401）⇒ **凡以"真实供应商调用成功"为成立条件的条目，仍不得计入成立**。
...
| **B** | ✅ **结构 ✅ / 因果 ✅（桩形态，2026-09-13）** | **结构**：`out\08-plaintext-scan-notmp.txt:2-5` ⇒ `FILE_HITS=0 / ENV_HITS=0 / PROC_ENVIRON_HITS=0`；`out\04-dsh-exec-inspect.json:178` 的 `DEEPSEEK_API_KEY` 为 **43 字符**（前缀 `D0DqBJ`，可与 `03-gw.log:2` 的 `MINT id=D0DqBJ..` 对应）⇒ **非**供应商密钥（38 字符，与上游报错 `****1fa0` 末位一致）；`:143-160` 的 `Mounts` 仅两个 `ro` 绑定。**因果：✅ 已取证（桩形态，见 §F12）** —— 以**本地桩上游**做「**唯一变量 = 宿主侧凭据**」的两轮对照：有凭据 → `UPSTREAM-RES 200` → turn `kind=completed`；**清空 → `UPSTREAM-RES 401` → turn 立即失败（`code=AUTH`）**。⚠️ **形态限定**：上游是**桩**，故只证「凭据位置 + 因果链」，**不含**「真实供应商是否接受真 key」。**原登记的两轮真实取证**（`02-driver.log` `AUTH 401` / `09-negative-case.log` `TRANSPORT`）**无法构造对照**，且 `TRANSPORT` **不等价于 B 的因果**（§F9.2 自注）。**注**：`04` 对应**网关在线**那轮（hostname `0a27ed22699f`） |
...
| **红线 1**（清空宿主侧凭据后 turn **仍成功**） | ✅ **未命中（桩形态，2026-09-13）** | 见 **§F12**：清空宿主侧凭据后 turn **立即失败**（`code=AUTH` / `status=401`），**没有**"仍成功" ⇒ 该红线**不成立**（此前为"无法判定"）。⚠️ **形态限定**同 B 行：上游是**桩**；**真实供应商口径下仍须复跑确认** |
...
**⇒ 汇总结论（❗ 2026-09-13 第二次更新：补入 §F12 桩形态取证）**：**A / C′ / D 的结论成立（各带上述局限）；成功态已在桩形态下取得（§F12），据此判据 B 的因果部分 ✅、红线 1 ✅ 未命中；F 仅 2/6 有条目证据，E / G 为部分；红线 2 / 3 未命中（但红线 3 的"镜像层"一项根本没测）。** **唯一仍未取证的 = 「真实供应商调用成功」**（＝凭据有效性，即那个 401，**不是架构问题**）。按本节「纪律」条，**只有静态证据、无运行期证据的条目不得计入成立** ⇒ **在有效凭据重跑之前，仍不得声称「容器内不持有模型密钥」的最终确认**（规格 §3.5 硬约束不变）。
...
| **B14** | ⚠️ **已大部分取证（2026-09-13 三轮迭代：§F9 实测 → 两轮复核下修 → §F12 桩形态补成功态）**：C2/C3 已实跑 —— **A / C′ / D 结论成立（各有局限）；成功态已取得（桩）；B 结构 ✅ / 因果 ✅（桩）；红线 1 ✅ 未命中（桩）、红线 2 / 3 未命中；F 仅 2/6 有条目证据、E / G 部分**。**唯一仍未取证 = 「真实供应商调用成功」**（＝那把凭据 401，**不是架构问题**）。**权威状态表见 §B14 的「当前取证状态」块。**〔原写「❌ 全未取证」，系 §F9 执行**之前**的陈旧表述〕 |
