...
- **遗留（属段二实施期，不影响本项"核对已闭环"的判定）**：① 未逐包比对许可证**正文**与声明值（583 条目；仅确认相关包随带 LICENSE 文件，`@img/sharp-win32-x64` 的 LICENSE 是写 NOTICES 的引用来源）；② ~~`THIRD-PARTY-NOTICES` 未产出~~ ⇒ **已产出（2026-09-19）**：根目录 [`THIRD-PARTY-NOTICES.md`](file:///d:/徐徐AI学习/公司工作台/THIRD-PARTY-NOTICES.md)，由 [`scripts/generate_third_party_notices.py`](file:///d:/徐徐AI学习/公司工作台/scripts/generate_third_party_notices.py) 可重跑生成，守护 `tests/test_third_party_notices.py`（12 条，含 4 轮反假）。**口径已扩到四类产物**（应用镜像 Python 53 项 / dsh npm 安装树 / 管理台 / 伴侣端 / 桌面端），共 **1152 个组件**；**LGPL 条目实为 4 条而非 2 条**——除 `sharp` 生态 2 条外，**应用镜像的 `psycopg-binary@3.3.5` 与 `psycopg-pool@3.3.1` 同为 LGPL-3.0-only**（本节此前未登记），义务已按 `sharp` / `psycopg` 两族分别写明「许可正文 + 可重链接 + 源码获取途径」；③ ~~镜像内二次扫描未做~~ ⇒ **仍未做（登记为边界）**：执行镜像（含 dsh）**尚未构建**（`WORKBENCH_EXEC_IMAGE_DIGEST` 留空、仓库内无该镜像定义）⇒ 本清单扫的是**锁文件与安装树**而非镜像层，**镜像内实际集合与清单的一致性未验证**；镜像定义冻结后须在镜像内复跑同一扫描（脚本支持 `--dsh-tree` 指向镜像内树）。

### B14 dsh 进程位置与模型凭据来源（U1）—— **取证清单与判据**（2026-09-13 新增）

> **⚠️ 2026-09-14 更新（须重取）**：§8 U24 接线后，**判据 D / E / F 的取证对象在生产装配下已存在**（`build_tool_execution` 在 ⑧ 前 mint、`ContainerSpec.environment` 把短期令牌放进容器 env），**不再是"不存在"**；但**本次仍用假 Docker 客户端 / 测试替身、未起真容器、未跑真实 dsh turn** ⇒ **D / E / F 的原始运行期证据（容器内 env / `/proc/*/environ` / 令牌来源 / 无供应商密钥）仍未重取**。**⇒ 现有 D / E / F 结论仍只作"原型形态下已验证"读，生产装配下须重取后方可声称"生产已验"**（详见规格 §8 U26 更新）。
...
| **C′** | **容器内不得存在指向供应商域名的连接**（**原判据 C「`--network none` 下 turn 仍成功」已作废** —— 2026-09-13 裁决**路径①**：它要求"模型调用完全在容器外发起"，与 `dsh --profile sdk` 的架构**不相容**，见 **§F8.2**）。取证方式：真实 turn 进行中，**在容器命名空间内**抓出站连接，确认**无**指向供应商域名的连接；**指向模型网关的内网连接是允许且预期的**。**〔2026-09-14 更新（§8 U20 方案 b-1）〕证据形态变更**：内网桥内新增**执行回调边车**（`POST /internal/exec-callback`）⇒ 允许且预期的内网连接**从「只有一条到网关」变为「到网关 + 到回调边车」**；判据仍是「**无供应商域名连接**」，但**不得再把「只有一条连接」当作 C′ 的成立形式**（旧口径见下行"当前取证状态"的 C′ 行）。**〔2026-09-14 二次更新（§8 U21 裁决「候选②」）〕** 边车改为**双宿**（执行内网 + 回调网），**执行容器仍不可达回调网**（新增运行期断言）；②④ 判定回到工作台。C′ 的成立口径**不变**（仍是「容器内**无**指向供应商域名的连接」），但**「仅一条连接」彻底不可比**。 | 容器内**无**供应商域名连接；**网关侧**可见到该出站连接 | 容器内出现指向供应商域名的连接 → 判 **A 亦不成立**（凭据与调用都在容器内） |
...
| **E**（**2026-09-13 按路径①重述**；**2026-09-14 二次重述：回调边车落地**） | 容器与宿主分别读 `/proc/<pid>/cmdline`、`/proc/<pid>/environ`、容器 spec | 「**argv / env 在 `ps` 里可见即为漏洞**」——**禁止项是「供应商密钥」**；`baseURL`（**仅指向网关的内网地址**）是**预期注入物**，不算违规。**〔2026-09-14 更新〕** 执行侧需回调工作台 ⇒ 容器内**允许**出现**执行回调边车的唯一端点地址**（预期注入物，与 `baseURL` 同口径）；**禁止项收窄为**：**不得**含**工作台 app 的其它控制端点 / 端口**、**不得**含**任何工作台凭据**（长寿命密钥 / 会话令牌 / 管理密钥），也**不得**含供应商密钥。容器内随行的**短期令牌**（回调 / 模型共用）受 §3.5 六条约束，**不算违规**。**〔2026-09-14 二次更新（§8 U21）〕** 边车**双宿**不改变本判据：容器内**仍只允许**回调边车的唯一端点地址；**工作台 app 的其它控制端点 / 端口与任何工作台凭据一律禁止**。 | argv / env / 容器 spec **均无供应商密钥**；**不含工作台 app 的其它控制端点 / 端口，也不含任何工作台凭据**；允许含回调边车唯一端点地址与短期令牌 | **供应商密钥**、**工作台凭据 / 工作台 app 控制端点**出现在任何 cmdline / env / 容器 spec → **立即判不成立** |
| F | **容器内唯一的短期网关令牌**：签发来源、使用面、生命周期、kill turn 后是否立即失效 | 短期凭据六条：每 turn 新铸 / 绑死租户·会话·代次 / 服务端为准 / constant-time 比对 / 终态同步吊销 / 孤儿上限〔**② 与 ④ 的校验点 ＝ 工作台控制面**（2026-09-13 用户裁决）；**网关数据面不做绑定**，网关侧 `bound` 仅作审计元数据 —— 见规格 §3.5 P1 注〕。**〔2026-09-14 更新（§8 U20 方案 b-1 → 同日 §8 U21 返工「候选②」）〕②④ 已在**工作台接收端点**落地**：边车**无状态纯转发** `POST /internal/exec-callback` → 工作台 `POST /api/v1/internal/exec-callback` 按令牌反查工作台自持绑定、`expected` 从服务端权威（会话当前代次）重建、`constant-time` 比对；不匹配 / 跨租户 / 旧代次 → `403`（不泄露存在性）。**边车→工作台 鉴权 = 预共享密钥、限流 = 100 rps**。**取证**：`tests/test_exec_callback.py`（含反假）、`tests/test_exec_callback_network.py`（执行容器不可达工作台、不可达回调网）；**残余**（两进程真实拓扑成功路径、`generation` 真实多 turn、多副本限流口径）见规格 §8 U21 | 满足六条 **且不持有任何供应商 API Key** | 拿到**开机固定的共享令牌** → 不成立 |
...
📌 **登记（原「未立判据」，2026-09-14 已覆盖并改写）**：**F 行原「现象」列写的是「执行侧回调工作台（导出产物 / 请求授权）时的凭据」，与六条本体（§3.5 P1 的**网关令牌**）不是同一个面**。2026-09-13 裁决将 **F 收敛到「网关令牌」**，**回调凭据另行登记、当轮不扩判据**。**〔2026-09-14 更新（§8 U20 方案 b-1）〕回调面已落地并有判据覆盖**：① **判据 E** 已重述为「容器内**允许**回调边车唯一端点地址；**禁止**工作台 app 的其它控制端点/端口与任何工作台凭据」；② **判据 F** 已并入 ②④ 在**回调边车**上的强制校验（上文 F 行）；③ **§5 用例 35** 已升级为**真端点端到端**（`tests/test_exec_callback.py`）并获得**真容器网络隔离**佐证（`tests/test_exec_callback_network.py`：执行容器**可达边车、不可达工作台**）。**⚠️ 残留（不得读成已完全覆盖）**：回调凭据的**来源 / 生命周期 / kill turn 后是否立即失效**（原登记待拟内容）**仍只覆盖「校验是否存在且匹配」**——**跨进程同步、`边车→工作台` 出向调用的鉴权与限流、多副本并发**均**未实现 / 未验证**（见规格 §8 U20 残余清单）。
...
| **C′**（容器内无指向供应商域名的连接） | ✅ **已取得证据**（⚠️ 采样方式有局限） | `02-driver.log:142-147` `connections:[{"remote":"172.22.0.2:8080","firstSeenMs":159}]`（**在线轮仅 1 条**，指向网关）；`:475` `supplierDomain:"BLOCKED TypeError: fetch failed"`；`09-negative-case.log:156` 停机轮 `connections: []`。**⚠️ 局限**：采样靠 `exec\driver.mjs` **轮询** `/proc/net/tcp*` 的 `st=='01'`（ESTABLISHED，约 150ms 一次）⇒ **短连接可能漏采**；且"`172.22.0.2` 即网关"**无 inspect 佐证**（属推断）。**〔2026-09-14 更新（§8 U20 方案 b-1）〕证据形态已变**：内网桥新增执行回调边车 ⇒ **允许的连接为「网关 + 回调边车」**；「只有一条连接」**不再**是判据 C′ 的成立形式（口径见上行 C′ 判据）。**本次未重取 C′ 原始日志** —— 因 2026-09-14 的边车落地**未触发真实 dsh turn 回调**（无容器内产物导出 / 请求授权），故现行原始日志仍为「仅 1 条到网关」；**待真实回调接入后须重取**。**〔2026-09-14 §8 U21〕** 边车双宿 + 判定回工作台**不改变**「无供应商域名连接」结论；「仅一条连接」**仍不可比**，原始日志仍待在真实回调接入后重取。 |
...
| **E**（2026-09-13 重述口径；**2026-09-14 二次重述**） | ⚠️ **部分** | argv / env / 容器 spec **均无供应商密钥** ✅；`DEEPSEEK_BASE_URL=http://gw:8080`（**仅网关内网地址**）为**预期注入物** ✅。**〔2026-09-14 更新（§8 U20 方案 b-1）〕** 原缺项「**不含工作台控制端点或其凭据**」**已改为可判定口径**（容器**允许**含回调边车唯一端点地址、**禁止**工作台 app 其它控制端点/端口与任何工作台凭据）——**但本次仍未用真实 turn 取证容器 env/spec 中的回调端点注入**（无回调触发方，见 C′ 行）。**〔2026-09-14 §8 U21〕** 边车双宿；本行口径不变（容器内允许回调边车唯一端点地址，禁止工作台控制端点 / 凭据）。 |
| **F**（短期凭据六条） | ⚠️ **仅 2/6 有条目证据，且均为单样本**（原表"4/6"已下修） | ① **每 turn 新铸** ⚠️ **仅 1 次样本**（`03-gw.log:2` `MINT … bound=turn-1 ttl=120s total=1`）② **绑死租户 / 会话 / 代次** ⚠️ **校验点已裁决（2026-09-13）＝ 工作台控制面；该处尚未实现 ⇒ 本条未闭环**：**网关数据面不做绑定是预期、不是缺陷**。静态证据：`bound` 只是宿主经 `/__mint` 传入的**标签**（`gw.js:40`，**缺省值 `unbound` ⇒ 连"必填"都不是**），数据面（`gw.js:56-71`）**只校验存在与过期、从不校验 `bound`**，`gw.js` 全篇**无租户 / 会话 / 代次概念**；**运行期反证**（`out\30-binding-gw.log`，2026-09-13）：4 个"应通过"用例（`bound=turn-1` / `turn-2` / `unbound` / **令牌自带 bound 与请求声明的绑定不匹配**）**全部 `502` = 通过认证**，2 个对照组（**伪造令牌** / **TTL 过期**）**均 `401`** ⇒ **判定链本身有效**，故"全部通过"**不是网关失能**。⇒ 网关侧 `bound` **仅作审计元数据**；**强制绑定须在工作台控制面**（执行侧回调时按令牌反查 `(tenant, session, generation)` 并 constant-time 比对）落地，**属段二实现期**〔**2026-09-14 更新：已在执行回调边车落地（`app/exec_callback`，§8 U20 方案 b-1），见上行 F 行判据与新"单端口回调面"；但无真实回调触发方（无容器内产物导出 / 请求授权）⇒ 仍非"运行期归档证据"**〕 ③ **服务端为准** ✅（凭据以网关内存 Map 为准，容器声明不参与）④ **constant-time 比对** ❌ **未实现**（`gw.js:58` 用 `tokens.get(tok)` Map 查找）⑤ **终态同步吊销** ❌ **无归档证据**（原表引用的 `REVOKE bound=turn-2 n=1` 与 `DENY … reason=unknown-token` **在 `out\` 内任何日志中都不存在** —— 全目录 grep 只命中 `gw\gw.js:47/60` 的**代码字面量**）⑥ **孤儿令牌上限** ❌ **未实现**（`gw.js` 无令牌计数上限）。⇒ **六条中真正有归档运行期证据的只有 ①（单样本）与 ③**。**〔2026-09-14 更新（§8 U20 方案 b-1）〕② 与 ④ 已在执行回调边车上实现（`expected` 服务端权威重建 + `constant-time`），判据见 `tests/test_exec_callback.py`（真端点 + 反假）**；**网关数据面不做绑定仍为预期**。**〔2026-09-14 §8 U21 返工〕** ②④ 判定现落**工作台**（`app/tool_execution/callback_guard.py`），边车纯转发；`边车→工作台` 的**鉴权（预共享密钥）与限流（100 rps）已实现**（端到端 / 单进程取证见 `tests/test_exec_callback.py`）。 |
...
- **事实**（第四轮复核 N3/P4；**第七轮 P3 后由 10 项增至 12 项**；**2026-09-13 裁决路径①后由 12 项增至 18 项**；**2026-09-13 段二-3 裁决 ⑥ 后增至 19 项**；**2026-09-14 方案 b-1 后增至 22 项；同日 §8 U21 返工后增至 23 项**；**2026-09-14 §8 U22 后增至 24 项**）：规格 §4 列出的 **24 项**新增配置（`WORKBENCH_AGENT_RUNTIME_BACKEND`、`WORKBENCH_EXEC_IMAGE_DIGEST`、`WORKBENCH_EXEC_WORKSPACE_ROOT`、`WORKBENCH_EXEC_TRUSTED_ROOTS`、`WORKBENCH_EXEC_TIMEOUT_SECONDS`、`WORKBENCH_EXEC_PIDS_LIMIT`、`WORKBENCH_EXEC_MEMORY_MB`、`WORKBENCH_EXEC_CPU_QUOTA`、**`WORKBENCH_EXEC_ORPHAN_LIMIT`（⑥ 孤儿上限，默认 `8`）**、`WORKBENCH_DSH_VERSION`、`WORKBENCH_ARTIFACT_EXPORT_ENABLED`、`WORKBENCH_BODY_ENCRYPTION_KEY`、**`WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`（§8 U22 新增：旧正文密文密钥，**多值、逗号分隔**、**启动时读入、无热轮换**、**默认留空 = 不使用旧密钥**）**、`WORKBENCH_BODY_CLEANUP_INTERVAL_SECONDS`、**路径①网关六项**：`WORKBENCH_MODEL_GATEWAY_BASE_URL`、`WORKBENCH_MODEL_GATEWAY_TOKEN_TTL_SECONDS`、`WORKBENCH_MODEL_GATEWAY_UPSTREAM_BASE_URL`、`WORKBENCH_MODEL_GATEWAY_UPSTREAM_API_KEY`、`WORKBENCH_MODEL_GATEWAY_UPSTREAM_TIMEOUT_SECONDS`、`WORKBENCH_MODEL_GATEWAY_MAX_RETRIES`、**执行回调边车四项**：`WORKBENCH_EXEC_CALLBACK_LISTEN_HOST`、`WORKBENCH_EXEC_CALLBACK_LISTEN_PORT`、`WORKBENCH_EXEC_CALLBACK_FORWARD_URL`、**`WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`（§8 U21 新增：边车→工作台预共享密钥）**）在 `app/settings.py`、`.env.staging.example`、`tests/test_env_templates.py` **三处零命中**〔**2026-09-13 更正：该"零命中"已不成立 —— 24 项已全部落上述三处，见本条末「进展」块**〕。
- **判据（实现前必须闭环）**：
  1. **24 项全部进 `app/settings.py`**（含类型与默认值；**`WORKBENCH_BODY_ENCRYPTION_KEY` 必填非空且不进仓库；`WORKBENCH_MODEL_GATEWAY_UPSTREAM_API_KEY` 同口径（上游供应商密钥，只在网关侧）；`WORKBENCH_BODY_CLEANUP_INTERVAL_SECONDS` 与 `WORKBENCH_EXEC_TIMEOUT_SECONDS` 的默认值须先定死——见规格 §8 U11/U16**；**新增的 `WORKBENCH_MODEL_GATEWAY_TOKEN_TTL_SECONDS` 须与之协调：TTL ≥ 执行超时**；**`WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS` 默认留空 = 不使用旧密钥（fail-closed）**）；
...
- **进展（2026-09-13）**：**判据 2 / 3 / 4 已闭环；判据 1 部分闭环。**
  - ✅ **判据 2**：**25 项**全部进 `.env.staging.example`（= 规格 §4 的 **24 项** + 网关侧 `WORKBENCH_MODEL_GATEWAY_MINT_SECRET`；含密钥类"必须由部署密钥系统注入、禁止在模板填写真实值"的声明）。**〔2026-09-13 第三次更新：19→20 —— 段 F 新增 `WORKBENCH_MODEL_GATEWAY_MINT_SECRET`（网关控制面密钥，只在网关侧）〕** **〔2026-09-14 §8 U21 更新：新增 `WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`（边车→工作台预共享密钥，两侧同值、缺失即拒、不进仓库）〕** **〔2026-09-14 §8 U22 更新：新增 `WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`（旧正文密文密钥，多值；模板中留空占位、禁止填真实值）〕**
  - ✅ **判据 3**：`tests/test_env_templates.py` **3 个守护用例**覆盖（① 清单↔实现的字段存在性；② 模板全覆盖 24 项；③ **默认值钉死 + `TTL ≥ EXEC_TIMEOUT` 硬约束 + fail-closed 项必须留空**）。**含反假测试**（首轮）：故意注释掉模板一行 + 把一个默认值改坏 ⇒ **恰好 2 个用例变红**（报错分别为 `['exec_cpu_quota']` 缺失、`181 == 180`），还原后复绿。**段二-3（2026-09-13）扩项**：⑥ 新增 `WORKBENCH_EXEC_ORPHAN_LIMIT`（默认 `8`），**三处台账同批同步**（`app/settings.py` / `.env.staging.example` / `STAGE2_SETTINGS_FIELDS`）并把项数由 **18 改为 19**，回归复绿。**§8 U21（2026-09-14）扩项**：新增 `WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`（fail-closed 默认留空），三处台账同批同步并把项数由 **22 改为 23**，回归复绿。**§8 U22（2026-09-14）扩项**：新增 `WORKBENCH_BODY_ENCRYPTION_PREVIOUS_KEYS`（多值、fail-closed 默认留空），三处台账同批同步并把项数由 **23 改为 24**（并把新项纳入「fail-closed 默认留空」清单），回归复绿。**〔2026-09-14 补注〕** `Settings` 段二字段同期 **24 → 25**（即 U22 新增 `body_encryption_previous_keys` 时更新；**乙口径**含网关侧 `mint_secret`）—— **甲口径（规格 §4 清单 / `STAGE2_SETTINGS_FIELDS`）仍为 24**；上句「项数由 23 改为 24」为**甲口径**。
  ...
  - ⚠️ **判据 1 部分闭环**：24 项**已全部进 `app/settings.py`**（含类型、默认值与 `ge/le` 边界；**不设裸名别名**，只认 `WORKBENCH_` 前缀）；**执行回调预共享密钥的 fail-closed 校验已落地**（边车缺 `WORKBENCH_EXEC_CALLBACK_SHARED_SECRET` **拒绝启动**；工作台接收端点缺失 / 不匹配一律 `403`）——但**其余"必填非空 / `base64` 32 字节 / `TTL ≥ 执行超时` 的启动期校验尚未实现** —— 其失败语义归 **§4.1.6-3「装配与失败语义」**，依赖目前**尚不存在**的装配组件（`tool_execution` / `run_records` / `tool_actions`）⇒ **随装配期一并落地**；此处**不提前造半成品**（避免与 §4.1.6-3 形成第二个事实源）。
  - **〔2026-09-14 方案 b-1 新增三项；同日 §8 U21 返工新增第四项 `WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`〕** 执行回调边车配置已同步三处台账：`app/settings.py`（`exec_callback_listen_host` 默认 `0.0.0.0` / `exec_callback_listen_port` 默认 `8081` / `exec_callback_forward_url` 默认留空=fail-closed / `exec_callback_shared_secret` 默认留空=fail-closed）、`.env.staging.example`（同名 `WORKBENCH_EXEC_CALLBACK_*` 四项，附"不得烤进镜像 / 留空即不启用"声明）、`tests/test_env_templates.py` 的 `STAGE2_SETTINGS_FIELDS`。**项数变更**：`STAGE2_SETTINGS_FIELDS` **19 → 23**（`Settings` 段二字段 **20 → 24**，含未纳入 STAGE2 清单的网关侧 `model_gateway_mint_secret`）；`test_stage2_settings_are_declared` / `test_staging_template_covers_every_stage2_setting` / `test_stage2_defaults_are_pinned` **已同批同步并复绿**。
...
- **状态（2026-09-13）**：**P0-1 / P0-3 / P0-4 已完成** —— P0-4（7 项四问）见 [自主可控方案 §9](file:///d:/徐徐AI学习/公司工作台/docs/key-dependency-autonomy-plan.md)，**P0-3 断网演练记录见同文 §10**；**P0-2 ⏸ 已挂起**（归档包**已就绪**：`dsh-0.1.5-rc.1-offline-archive.tar.gz`，52 MiB，sha256 `3ceb0178…e811`；**仍存于本机**，见下条）。
  - **按判据逐条对齐**：**① 未闭环**（"落到内网受控位置"未做）—— **〔2026-09-14 进展〕清单与搬运脚本已就绪，待内网侧执行**：① 本机**实算复核** `sha256 = 3ceb01786d246997bf874ddb57e429e43a2e4f8d618166bab1265971c492e811`（**与登记值前缀 `3ceb0178` / 后缀 `e811` 一致 ✓**；大小 **54,509,877 字节** ≈ 52 MiB，与登记一致）；② 搬运 + 逐字节校验清单（含可直接执行的 bash 段、`set -euo pipefail`）见**仓库外** `D:\徐徐AI学习\_b16-archive\B16-1-archive-manifest.md`；③ **本机够不到 NAS** —— `/mnt/nas/dsh-archive` 不可达（无挂载、无网络盘映射、WSL 仅 `docker-desktop`）⇒ **须由内网侧执行 `cp` + `sha256sum -c` 并留存原始输出**。**⇒ 在拿到该原始输出之前，① 保持未闭环。**
  - **〔2026-09-14 判据① 只读核查 + 用户裁决〕** **核查结果（归档包内部）**：**版本钉死 ✅** —— lockfile 主包 `@deepseek-ai/dsh` 精确为 `0.1.5-rc.1`（注：同生态其它 `@deepseek-ai/*` 子包多为 `0.1.5-rc.2`，**主包为 rc.1**）；**不依赖上游可访问 ✅（静态证据充分）**—— 完整本地 `_cacache`（502 块 / 55.8M，**无 `node_modules`**）+ lockfile + 演练日志（`npm ci --offline` → **522 包**、`NPM_CI_EXIT=0`、`--network none`）；残留的 `resolved: https://registry.npmjs.org/…`（**582 处**）属 **npm 元数据**、**非运行期取件依赖**；**敏感信息 0**（私钥 / `sk-` / `AKIA` / `ghp_` / `.env*` / `token|secret|credential` 文件名**全部 0**；`expected-dump.yaml` 的 7 处命中逐条核为**良性配置键**）。
  - **两点裁决（2026-09-14 用户）**：① **「含校验和」口径 = 同目录一并交付** —— **归档包内部确不含任何校验和文件**（5 个非 cache 文件无一为校验和文件；`package-lock.json` 的 582 个 `integrity` 是**依赖包自身**哈希、**不能 `sha256sum -c`**）；校验和实际位于**包外同目录** `MANIFEST.md §3` 与 `make-archive.log`。**⇒ 交付时须把校验和清单与归档物一并放入受控位置**（本轮加固脚本会在目标目录生成 `SHA256SUMS` + `PROVENANCE.txt`）。② **镜像纳入受控位置** —— 判据① 原文为"源码 / **镜像**归档"，而基础镜像 `node-22-slim-image.tar`（**82,586,112 字节**，sha256 `d19c3368119d57164229e86d665b3b67ccf38353486fc0c750a0956f171e7029`）**是包外的独立文件** ⇒ **须与归档包一并交付**，否则"镜像归档"未达成。
  - **⇒ 交付集合（3 件 + 脚本产物）**：`dsh-0.1.5-rc.1-offline-archive.tar.gz`（**54,509,877 字节**，sha256 `3ceb01786d246997bf874ddb57e429e43a2e4f8d618166bab1265971c492e811`）、`node-22-slim-image.tar`（**82,586,112 字节**，sha256 `d19c3368…7029`）、`MANIFEST.md` + `make-archive.log`（出处与校验）；**并由脚本在目标目录生成 `SHA256SUMS`（覆盖全部文件）+ `PROVENANCE.txt`**。
  - **仍未验证**：**未独立复跑 `npm ci --offline`**（仅有包内自述作为间接证据）；**未核 `node-22-slim-image.tar` 的内容**；**内网位置仍不可达** ⇒ **① 仍保持未闭环**。；**② ✅ 已闭环（对 `dsh`）**——`--network none` 下纯离线装出 522 包、最小链路 exit 0 / 352 行、**输出与期望逐字节一致**，原始输出已留存；**③ ✅ 已闭环**——7 项退出方案（候选 ≥1、迁移成本、触发条件）已补，**但候选许可多数"待核"**。
...
> **⚠️ 2026-09-14 更新（须重取）**：§8 U24 接线后，**判据 1 的 D 行的取证对象在生产装配下已存在**（生产会 mint + 把短期令牌放进容器 env）⇒ **判据 1 / 3 须在真实回调接入后重跑**（本次**未重跑**：仍用假 Docker 客户端 / 测试替身，未起真容器、未跑真实 dsh turn）。**「无供应商密钥」结论不受影响**（`dsh.build_token_env` 对供应商密钥做拒绝性自检 + 剔除禁止字段名，规格 §8 U24 / U26 已登记）；**但「仅一条连接」仍不可比**，原始日志仍待重取。
...
- **状态（2026-09-13 首次检测 → 🟡 部分闭环）**：
  ...
  - **衍生待核项 —— 已核实并排除（2026-09-13）**：本次检测顺带发现 **`WORKBENCH_EXEC_IMAGE_DIGEST` 的生产配置为空**（默认 `""` + 模板留空），**一度疑为"配置里执行镜像等于没钉"，经代码核实系误判 —— 它是刻意的 fail-closed 默认，不是缺口**：① `app/tool_execution/executor.py:187` 在 digest 为空时**拒绝启用真实执行**（原文："未配置执行镜像 digest WORKBENCH_EXEC_IMAGE_DIGEST，拒绝启用真实执行"）；② `app/runtime/adapters/dsh.py:215-217` 同时校验「为空 ⇒ 拒」与「不含 `@sha256:` ⇒ 拒」，**digest 钉死是强制的**；③ `app/settings.py:443-446` 的默认值取舍注明"能 fail-closed 的一律留空或关闭"（2026-09-13 用户裁决）。**⇒ 空值 = 未配置 ⇒ 真实执行被拒（fail-closed），与 §3.3 一致。**（教训：本次先登记"疑似缺口"、后核消费点 —— **应先看消费点再定性**。）
