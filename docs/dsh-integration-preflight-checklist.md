...
- **进展（2026-09-13）**：**判据 2 / 3 / 4 已闭环；判据 1 部分闭环。**
  - ✅ **判据 2**：**24 项**全部进 `.env.staging.example`（= 规格 §4 的 **23 项** + 网关侧 `WORKBENCH_MODEL_GATEWAY_MINT_SECRET`；含密钥类"必须由部署密钥系统注入、禁止在模板填写真实值"的声明）。**〔2026-09-13 第三次更新：19→20 —— 段 F 新增 `WORKBENCH_MODEL_GATEWAY_MINT_SECRET`（网关控制面密钥，只在网关侧）〕** **〔2026-09-14 §8 U21 更新：新增 `WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`（边车→工作台预共享密钥，两侧同值、缺失即拒、不进仓库）〕**
  - ✅ **判据 3**：`tests/test_env_templates.py` **3 个守护用例**覆盖（① 清单↔实现的字段存在性；② 模板全覆盖 23 项；③ **默认值钉死 + `TTL ≥ EXEC_TIMEOUT` 硬约束 + fail-closed 项必须留空**）。**含反假测试**（首轮）：故意注释掉模板一行 + 把一个默认值改坏 ⇒ **恰好 2 个用例变红**（报错分别为 `['exec_cpu_quota']` 缺失、`181 == 180`），还原后复绿。**段二-3（2026-09-13）扩项**：⑥ 新增 `WORKBENCH_EXEC_ORPHAN_LIMIT`（默认 `8`），**三处台账同批同步**（`app/settings.py` / `.env.staging.example` / `STAGE2_SETTINGS_FIELDS`）并把项数由 **18 改为 19**，回归复绿。**§8 U21（2026-09-14）扩项**：新增 `WORKBENCH_EXEC_CALLBACK_SHARED_SECRET`（fail-closed 默认留空），三处台账同批同步并把项数由 **22 改为 23**，回归复绿。
