...
| **C′** | **容器内不得存在指向供应商域名的连接**（**原判据 C「`--network none` 下 turn 仍成功」已作废** —— 2026-09-13 裁决**路径①**：它要求"模型调用完全在容器外发起"，与 `dsh --profile sdk` 的架构**不相容**，见 **§F8.2**）。取证方式：真实 turn 进行中，**在容器命名空间内**抓出站连接，确认**无**指向供应商域名的连接；**指向模型网关的内网连接是允许且预期的**。**〔2026-09-14 更新（§8 U20 方案 b-1）〕证据形态变更**：内网桥内新增**执行回调边车**（`POST /internal/exec-callback`）⇒ 允许且预期的内网连接**从「只有一条到网关」变为「到网关 + 到回调边车」**；判据仍是「**无供应商域名连接**」，但**不得再把「只有一条连接」当作 C′ 的成立形式**（旧口径见下行"当前取证状态"的 C′ 行）。 | 容器内**无**供应商域名连接；**网关侧**可见到该出站连接 | 容器内出现指向供应商域名的连接 → 判 **A 亦不成立**（凭据与调用都在容器内） |
...
- **进展（2026-09-13）**：**判据 2 / 3 / 4 已闭环；判据 1 部分闭环。**
  ...
  - **〔2026-09-14 方案 b-1 新增三项〕** 执行回调边车三项已同步三处台账：`app/settings.py`（`exec_callback_listen_host` 默认 `0.0.0.0` / `exec_callback_listen_port` 默认 `8081` / `exec_callback_forward_url` 默认留空=fail-closed）、`.env.staging.example`（同名 `WORKBENCH_EXEC_CALLBACK_*` 三项，附"不得烤进镜像 / 留空即不启用"声明）、`tests/test_env_templates.py` 的 `STAGE2_SETTINGS_FIELDS`。**项数变更**：`STAGE2_SETTINGS_FIELDS` **19 → 22**（`Settings` 段二字段 **20 → 23**，含未纳入 STAGE2 清单的网关侧 `model_gateway_mint_secret`）；`test_stage2_settings_are_declared` / `test_staging_template_covers_every_stage2_setting` / `test_stage2_defaults_are_pinned` **已同批同步并复绿**。
