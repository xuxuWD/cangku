...
**⇒ 汇总结论（❗ 2026-09-13 第三次更新：§F13 真实供应商收口）**：**A / C′ / D 结论成立；B 结构 ✅ / 因果 ✅；红线 1 ✅ 未命中；红线 2 / 3 未命中（红线 3 的"镜像层"一项仍未测）。** **七条判据与三条红线至此全部有证据**（成功态先桩后真实，见 §F12 / §F13）。**仅存两项限定**：① **F 仅 2/6 有条目证据**（②④⑤⑥ 属段二实现期 ⇒ 规格 §8 **U17**）；② **容器形态未加固** ⇒ 门禁 **§B17** 独立成立、**阻断上线**。按本节「纪律」条，**只有静态证据、无运行期证据的条目不得计入成立**。
...
✅ **2026-09-13 收口（原挂起已解除）** —— 用户更新凭据后，用**同一脚本**把上游换成 `https://api.deepseek.com` 复跑两轮（见 **§F13**）：**轮 A `UPSTREAM-RES 200` + turn `kind=completed`；轮 B 清空凭据 `UPSTREAM-RES 401` + turn 立即失败 `code=AUTH`** ⇒ **「真实供应商调用成功」已取得，B14 七条判据与三条红线至此全部有证据**。**仍存的两项限定**：① 容器形态**未加固** ⇒ 门禁 **§B17** 独立成立、**阻断上线**；② **F 的 ②④⑤⑥** 属段二实现期（见规格 §8 **U17**）。
