...
- **未拿到契约前**：**不做占位实现**（本计划维持「外部依赖阻塞」，不臆造接口形状）。契约索取表见 `docs/geo-contract-request.md`（7 类契约项 + 填写模板 + 开工前提）。

### 项 7 · 真实 staging 与真实平台账号验收

- **目标**：证明真实平台账号可被工作台安全使用（凭据注入、账号可达、权限最小），**不含发布动作**（发布在项 9）。
- **需要的输入**：staging 独立主机与凭据（依赖项 1）；真实平台账号与发布授权；回执核对人。
- **接入资料与索取表**：`docs/platform-account-onboarding.md`（平台 = **公众号**；含工作台发布协议、官方发布链路与硬约束、账号侧 7 项索取、两条落地路径）。**关键前置**：公众号账号主体类型与认证状态决定发布接口能否调用。
