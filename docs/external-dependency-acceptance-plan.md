...
- **需先补的代码（阻塞项）**：**网页抓取与公众号自动发布均未实现** —— 内容工作台 Alpha 明确「不抓取网页、不调用真实模型、不会自动发布」（`README.md`）。真实模型接入代码**已具备**（`OpenAICompatibleContentGenerator` + `WORKBENCH_CONTENT_GENERATION_BACKEND`），只缺真实密钥联调。
  需要新增：① 抓取器（含 robots 遵守、域名白名单、限速、合规边界）；② 发布器（幂等、回执核对、失败进人工接管、**绝不自动重发**）；③ 内容安全评估用例（提示词注入 / 数据泄露 / 越权输出）。
