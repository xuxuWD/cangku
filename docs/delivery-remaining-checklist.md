...
- [ ] 10.7 是否存在**用户数据导出 / 删除**路径（宪法九章）；若无，明确归档与保留策略的补位口径
      ...
      - **未验证（不得读成已验）**：① staging / 生产**未验收**（属组 1 外部任务）；② 管理台**前端未接入**取回端点（本机未跑前端套件）；③ 真库清理用例只覆盖「删过期行」语义，**未做跨租户大规模数据演练**；④ CI **不覆盖真实 beat 排程下的「到点清理」现场**；⑤ 差距面（其余 14 类导出类别 / 删除清场扩围 / 确认人 / 保留策略执行器 / 用户级导出删除）**本轮明确不改码**，属后续专项；⑥ 本批**尚未推送 ⇒ CI 未取证**（待提交推送后销账）。
