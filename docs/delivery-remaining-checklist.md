...
- [ ] 10.1 审计是否有 **DELETE / 清理路径**（`app/audit/store.py` 与数据库层是否 REVOKE）——判据：**可删即不合规**
      ...
      - **数据库层差距（未做，不得读成已验）**：全仓 `*.sql` 无 `REVOKE` / `GRANT`、`migrations/010_audit_log.sql` 无删除/改保护 ⇒ 按当前部署口径（应用 DSN 兼作迁移账号＝表属主）账号天然有 DELETE / UPDATE 权限 ⇒「不可篡改」目前靠**应用层无路径 + 检测层**，**不靠 DB 硬约束**；且 **REVOKE 生效前提是账号分离**（属主角色 vs 应用 DML 角色），非单条 SQL 可毕 ⇒ 处置待拍板。
...
- [ ] 10.8 「**发起人不得自审**」在**所有审批入口**都成立（含段二新增的对话入口）
