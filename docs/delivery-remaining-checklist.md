...
**转交与催办材料（2026-09-16 备）**：`docs/infra-input-request.md` §7 新增「转交说明与催办附言（发送人自用）」——含可复制的催办附言模板（回传渠道 / 期望回执时间留空待填）、发出人自查三条红线，以及「先回 I1–I6 + I10 即可先解锁只读核验面」的分批口径；**转交状态＝材料已备、待发出**（发出与回执状态由用户回填；回执到达后按索取表 §6 流程登记）。
