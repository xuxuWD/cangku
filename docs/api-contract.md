...
**删除清场口径（B-3 + B1 + B2/B3，2026-09-19）**：租户删除是**物理清场**（软删语义只留给正常业务）。当前清场面为**会话层、任务与运行层、记忆层、技能层、知识治理层**五处，逐面调用各层 `delete_all_for_tenant`，并在审计 `cleared_categories` 里如实记录真正清到的面；**未注入的面不参与清场、也不出现在该列表**（不假装清过）。**「整层」= 该层在迁移里的全部租户级表**，不是只清主表：记忆层 = 事实 / 规则 / 身份类画像三张（`029`），技能层 = 技能包 / 数字员工绑定两张（`030`），知识治理层 = 知识文档一张（`032`）——2026-09-19 修复前两处只清主表，导致 `cleared_categories` 面名齐全却仍有整租户行残留（规则 / 画像 / 绑定），现已按整层清场并落守护用例。**会话层（B1）= 六张表**：`execution_idempotency` → `conversation_stream_frames` / `conversation_stream_state` → `conversation_members` → `conversation_messages` → `conversations`（该顺序由**真库外键**决定：幂等行同时引用会话行与消息行、流帧/流态/消息均引用会话行，且这些外键**无级联**；成员表虽有级联仍显式删以保证逐表可数。见 `app/conversation/purge.py`）。**任务与运行层（B2+B3）= 一面**：与保留策略执行器**共用同一套删除 SQL**（`app/commercial/retention.py` 的 `purge_all_for_tenant`，全龄 ⇒ 不分年龄），覆盖 `workbench_tasks`（带「其运行全部已删」防孤儿谓词）、提案域（`plan_proposals` / `orchestration_proposals`；`workbench_plan_versions` 是**商业化套餐版本目录**，**不清理**）与整个运行域（先子后父：产物登记 → 验收决议 → 沉淀链接 → 工具动作 → 执行幂等 → 运行时状态 → 运行记录；**并额外清 `workbench_runtime_events`**——按龄清理不碰它（属 `events` 面、由全局 30 天清理器负责），但租户销毁时它属租户数据）。**两批合为一面的理由**：任务删除以「其运行已全部删除」为前提（`run_records.task_id` 无外键），拆成两面会留下「面名齐全、任务却按谓词被跳过」的假清场空间。⚠️ 该面**销毁**任务域事件流（`audit_events` 随任务级联）与运行域证据（工具执行 / 验收决议 / 沉淀链接 / 产物登记）——已裁决接受。**清场未覆盖的租户级表**（CRM、账号、收件箱、授权绑定等）按批次继续扩围（`docs/superpowers/specs/2026-09-19-tenant-purge-expansion-design.md` 的 B4→B5），对象存储文件 / 向量索引 / 缓存另立专项。worker 到期执行时若管理员尚未显式确认，则**按申请自动确认**（确认人 = 请求人，写同一审计动作 `commercial.deletion.confirmed`），避免冷静期长达数十天导致删除静默卡死。
...
- **人工在环**：提交 → `ceo` / `super_admin` 审核（提交人不能审自己的提交）→ 启用；**不允许用户自传技能包直接生效**。
  （2026-09-20 更正角色口径：原只收 `super_admin`，与 `permission-matrix.md` §3「技能：复核/启用/停用」行冲突，已按矩阵对齐 `ceo` + `super_admin`。）
...
- **使用权的传递上限（O3 · 派生授权 —— 2026-09-24 用户裁决，⚠️ 本条须先于实现落定；宪法 4.1「权限模型属地基级」）**：会话绑定的 `agent_key` 决定「这条会话用哪个数字员工」，因此**会话发起人**对该 `agent_key` 的**当前**授权就是这条会话的**使用权上限**。**执行期每次重判**，以下**两条同时成立**才放行：
  1. **发起人当前**对该 `agent_key` 享有 `owner ∪ use`（**或**发起人是 `super_admin`）；
  2. **发言者**是该会话的本人或 `write` 成员（＝既有发言写权限，本闸门**不放松**它）。

  - **⇒ 使用权随会话传递，上限为发起人当前权限**：`use` 档的派活权经会话传递给被点名的 `write` 成员，但**上限永远是发起人此刻的权限** —— 发起人失去 `use`（撤销共享、员工被改归他人）后，成员**立即**失去传递权（下一个请求即 `403`）；改造前已建、绑了他人 `agent_key` 的**存量会话**，**下次执行即被拦**（执行期重判，不是建会话时的一次性校验）。（员工被**停用**属另一轴：按下方闸门次序硬约束，命中「存在且启用 ⇒ `422`」，**不是**本条的 `403`。）
  - **拒绝 ⇒ `403`**，且**零落库**（不建承载任务 / 运行 / 消息 / 幂等行，不调工具）。文案**不区分**「员工不存在 / 无 `use` 档 / 撤销后失效」，统一为「没有使用该数字员工的权限」；会话本身对该发言者不可见时仍 `404`（沿用既有「不泄露存在性」口径）。
  - **⚠️ 推进处（审批决议）同样重判 —— 2026-09-25 补，堵「待批路径绕过执行期重判」**：「执行期每次重判」**不止**首次执行口。`POST /api/v1/runs/{run_id}/approvals/{approval_id}/approval` 在**落决议之前**，由 `run_id` 经幂等行反查会话，按上条**重判发起人当前**的 `owner ∪ use`（或超管）—— 不成立 ⇒ `403`，**不落决议、不调 `tool_execution_service.resume`、不真实执行工具**（零副作用）。否则撤销 `use` 后，此前已进入待批的动作仍会经决议端点被批准并真实执行工具 ⇒ 本条的「发起人失去 `use` 后成员立即失去传递权」在待批路径上不成立。反查不到幂等行 / 会话（非对话触发的运行）按既有边界**不拦截**；校验本身失败 ⇒ `503` fail-closed（与既有 `ask` 模式判定同口径，见「P2c-4」节）。
  - **⚠️ 与上一条「以**发言者本人**走既有**全部**闸门」的关系（**不是冲突 —— 是两条不同的授权轴**）**：上一条管的是「**以谁的身份执行**」（`requested_by` / 审计 `actor_id` / 工具白名单 / 自治三档 / 审批 / `critical` / 不自审），**本闸门不改动它**；本闸门管的是「**这条会话能不能用这个数字员工**」，判据落在**发起人**。两件事**同时成立才执行**。
  - **⚠️ 闸门次序（硬约束）**：本闸门**必须排在「`agent_key` 存在且启用」校验（→ `422`）之后** —— 否则「员工已停用」会从契约规定的 `422` 变成 `403`（见「工具执行（P2a 段二）」变更点 2）。
  - **⚠️ 涉及落库路径的边界（本轮实测登记）**：全仓 `task_store.create` **共 3 处** —— `app/main.py`（目录面，已有 `ensure_can_dispatch`）、`app/conversation/execution.py`（**本闸门的落点**）、`app/content/service.py`（`employee_key` **硬编码** `"content-writer"`，调用者**不可选目标** ⇒ 不构成本条的越权面，单列另行裁决）。
- **边界**：跨租户 / 部门级 / 全租户 / 公开链接**均不做**；成员**不得**归档、改模式、删会话、增删成员；**不做实时在线态**（参与者列表 + 最近活动时间为**非实时**呈现）。

## 桌面执行端：授权 / 事件 / 结算（B1 · **契约先行 · 未实现**）

> **⚠️ 状态（2026-09-24）：本节为「契约先行」——三个端点均**尚未实现、未装配、未验收**。`app/main.py` 现无 `/execution/*` 任何路由。**
> **依据**：[执行模型（桌面 + 服务端并存）与客户端合并 立项规格](superpowers/specs/2026-09-22-desktop-execution-and-client-merge-design.md) §2.2（三阶段回传）/ §2.3（断网）/ §2.4（九步闸门拆分）/ §2.5（D1/D2/D3）。该规格已于 2026-09-24 **评审通过**（`decision-log.md` **D-058①**）。
> **按该规格 §7 建议顺序第 3 步**：**先落本节契约，评审通过后才写实现**。**实现前不得据本节声称已具备桌面执行能力。**
> **⚠️ 字段级细节的边界（照实标，供评审员重点核）**：规格 §2.2 只定死了 **路径 / 用途 / 三段分离语义 / 令牌须含「授权范围 · 有效期 · 幂等键」/ 结算失败 ⇒ 标记「未结算」/ 幂等键贯穿三段**。**下列具体字段名、状态码分支与错误文案为本轮提案，不在已评审规格内，须评审确认。**

**铁律**：`app/audit/` 是唯一审计真源。桌面执行端**不自行判定权限（D1）、不持有长期凭据（D2）、不落独立审计（D3）**。

| 段 | 端点 | 可丢性 | 说明 |
| --- | --- | --- | --- |
| ① 授权（执行前） | `POST /api/v1/execution/authorize` | **不可丢** | 服务端跑闸门 ①–⑦，下发短期执行令牌 |
| ② 执行中 | `POST /api/v1/execution/{execution_id}/events` | **可丢** | 过程记录；允许延迟、批量、缓冲，**丢失不致命** |
| ③ 结算（执行后） | `POST /api/v1/execution/{execution_id}/settle` | **必须成功** | 审计摘要；**失败 ⇒ 任务标记「未结算」**，不假装成功 |

> **设计理由（规格 §2.2 原话）**：**过程可丢、结论不可丢——审计要求的是结论完整，不是过程完整。**

### `POST /api/v1/execution/authorize`

**这是桌面任务的闸门入口**。按规格 §2.4，桌面任务的九步闸门**「判定」与「记账」仍在服务端，只有第 ⑧ 步「执行」落到桌面** ⇒ 本端点的**判定语义与状态码直接沿用上节「九步闸门 → HTTP 语义」表**，**不另造一套**。

- 请求头：**`Idempotency-Key`（必填）** —— 幂等键贯穿三段，防重复授权（规格 §2.2）。缺失 ⇒ `422`。
- 请求体（提案）：`{"task_id": string, "run_id": string, "tool_key": string, "params": object, "workspace_root": string}`；未知字段 `422`。
- **判定结果 → 状态码**（逐条对齐既有闸门表）：

| 结果 | 状态码 | 响应体（提案） |
| --- | --- | --- |
| ①–④、⑦ 全过，⑤ 判定**无需**审批 | `200` | `{"execution_id","token","expires_at","authorized_scope","idempotency_key"}` |
| ⑤ 定档为**需审批** | `202` | `{"execution_id","status":"pending_approval","run_id","approval_id"}`（复用既有 `/runs/{run_id}/approvals/*`） |
| ① 工具不在白名单 | `422` | 「未知工具」 |
| ② 参数 schema 不符 / 含未知字段 | `422` | 「参数不符合要求」 |
| ③ 路径越界（realpath 含 `..`、符号链接） | `403` | 「路径不在允许范围内」 |
| ④ 危险命令闸门 | `403` | 「该命令被禁止执行」 |
| ⑥ 审批落库失败 | `503` | — |
| ⑦ 授权位失效 / 摘要不一致 | `409` | 「授权已失效，请重新审批」 |

- **令牌内容**：`token` = 短期执行令牌，须含**授权范围**（工具 + 规范化参数摘要）、**有效期**、**幂等键**（规格 §2.2）。**桌面不持有长期凭据（D2）**；**令牌过期后桌面不得自行续期**，只能重新走 ①。
- **不返回**：宿主真实路径（对外用虚拟化路径）、凭据 / 认证头 / Cookie、其它租户数据、内部堆栈（沿用「响应与不返回」节）。
- **权限**：仅对**已登录且持有该任务 / 运行读权**的账号开放；`customer_admin` 维持既有 `403`。**需审批的动作仅 `ceo` / `super_admin` 可决议，且发起人不得自审**（沿用既有口径）。
- **跨租户 / 越权一律 `404`**（不泄露存在性）。

### `POST /api/v1/execution/{execution_id}/events`

- **语义**：桌面把执行过程事件回传，服务端据此推进既有帧通道（P2b / P2c-2）。
- **可丢**：允许**延迟、批量、缓冲**；**丢失不致命**（规格 §2.2）。
- 请求体（提案）：`{"events": [{"seq": int, "kind": string, "payload": object}], "batch_id": string}`；未知字段 `422`。
- **响应**：`202 {"accepted": int}`。**不做**逐条错误返回——**过程记录不构成审计事实**。
- **鉴权**：`Authorization: Bearer <① 下发的短期令牌>`；令牌无效 / 过期 / 跨租户 ⇒ `403`（固定文案，不泄露存在性）。
- **⚠️ 未定（须评审）**：事件 `kind` 取值域是**直接复用** P2b 已冻结的帧 `kind` 集合，还是另立子集？规格未写，**本契约不预判**。

### `POST /api/v1/execution/{execution_id}/settle`

- **语义**：桌面提交**审计摘要**；服务端**一次性、幂等**写审计（规格 §2.2）。
- **必须成功**：**失败 ⇒ 该任务标记「未结算」**，且**在界面上显式可见**（不假装成功）。
- 请求体（提案）：`{"status": "succeeded"|"failed", "reason"?: string, "result_digest": string, "started_at": string, "finished_at": string}`；未知字段 `422`。
- **响应**：`200 {"settled": true, "audit_id": string}`；**同一幂等键重放 ⇒ 返回既有结果，不二次写审计**。
- **审计**：写**既有 append-only 审计**（**不建新表**）。**动作码与明细键须评审确定** —— 提案复用 `tool.executed` / `tool.blocked`，明细走既有 `ALLOWED_DETAIL_KEYS` 白名单，**`reason` 用受控枚举码**（沿用既有 9 值集合），**不落正文、命令行原文、真实路径**。

### 断网口径（规格 §2.3，**用户已裁 = B**）

- 断网 ⇒ 桌面进入**只读模式**：可看历史、可浏览，**不可执行需结算的动作**。
- 断网期间的本地操作**不入账、不产生副作用**；恢复后由用户**手动重试**。
- **理由（规格原文）**：方案 A 的本地暂存补传会造成**审计空窗期** —— 「断网期间那台机器上发生了什么」无法回答，而这恰是审计要防的事。

### 本节已知限制与待评审（如实登记）

- **⏳ 本节点全部未实现**（无路由、无迁移、无测试）。
- **🔴 R1 — 规格 §6 自述的最大技术风险**：闸门第 ③ 步 realpath 在服务端做，但桌面执行的**实际路径在用户机器上** —— 挂载点、大小写、符号链接、Windows 盘符差异会导致**服务端视图 ≠ 桌面视图**。**缓解方案待设计**（服务端下发「工作区根」契约 / 双端各校验一次）。
- **⚠️ 本节字段级细节为本轮提案**（含状态码分支与错误文案），**须评审确认后才可作为实现依据**。
- **⚠️ 前置**：按规格 §7，本节通过后**仍须**先落 `api-contract.md` 的目录口径变更（见下节 C1–C5）方可进入实现。

## 数字员工归属与共享（B2 · **契约先行 · 未实现**）

> **✅ 状态（2026-09-24 更新，`decision-log.md` **D-061**）：本节已**落地** —— 原「契约先行 · 未实现」表述**已失效**，保留于标题以留痕。**
> **已落地**：迁移 `045_digital_employee_trunk`（`b0ae9e4`）· 权限闸门拆分三档 + 共享三端点 + 视图只增两字段（`e5626aa`）·
> 前端接线（`ed5f147`）· 真库回归 `tests/test_workforce_directory_gate_split_postgres.py`。专项材料见 [`gate-split-review-2026-09-24.md`](contracts/gate-split-review-2026-09-24.md)。
> **⚠️ 仍未落地（如实登记）**：**6 类预置模板的种子数据**（N-a 的存储形态未定，留 `046_*`）；
> **`visibility` 列已落库但对内恒为 `private`**（未接线，与 D-058③ 砍第三档一致）；
> **前端未真机走查**（四态在浏览器里的呈现未取证）。
> **⚠️ 一处与下文冲突须一并读**：下文「背景（实测缺口）」与「两处数据模型变更」写的是**改造前**的实测状态
> （「仅 `super_admin`」「迁移 045 尚未编写」等），**现均已不成立** —— 保留为历史留痕，勿据其判断现状。
> **依据**：[B1 规格](superpowers/specs/2026-09-22-desktop-execution-and-client-merge-design.md) §3.3①②④ / §3.4 · [B2 施工方案](contracts/b2-construction-plan-2026-09-24.md) §4（C1–C5 经 `api-contract.md` 核对后的**真实归类**）· `decision-log.md` **D-057 / D-058③④**。
> **⚠️ 权限模型变更属「地基级」（宪法 4.1）**：本节 **C1 / C3 / C4** 触及**目录可见性**与**闸门拆分** ⇒ 按 B1 §3.4 评审附条件，**须单列评审后才可实施**。

**背景（实测缺口）**：`workbench_digital_employees` 只有 `created_by`、**无 `owner_user_id`**，**无共享机制**，目录接口**仅 `super_admin`**（`_require_workforce_directory_admin`，`app/main.py:1362`，**实测守着 9 个端点**）⇒ 普通员工看目录得到 `403`。

**两处数据模型变更**（DDL 见 B1 §3.3；**迁移 045 尚未编写**）：

- `workbench_digital_employees` 增 `visibility`（`private` / `shared`，**默认 `private` —— 最保守，不因迁移而扩大可见性**）与 `owner_user_id`（**存量回填 `created_by`** 后 `SET NOT NULL`）。
- 新增 `workbench_employee_shares`（两档 `read` / `use`，**刻意对齐** P2c-6 会话成员表的两档形态，不引入第二套心智模型）。

> **⚠️ 迁移前必做（B2 施工方案 §5）**：`workbench_digital_employees.created_by` 实测为 **`TEXT NOT NULL`** ⇒ 那句 `SET NOT NULL` **永远不会因 NULL 而失败**；但若存量行是**空字符串**，回填后 `owner_user_id` 就是空串，`SET NOT NULL` **拦不住空串** ⇒ **迁移"成功"了、数据是垃圾的**，要等线上"某个员工看不见自己的员工"才发现。
> **须先在真库跑**：`SELECT count(*) FROM workbench_digital_employees WHERE created_by IS NULL OR created_by = '';`
> **若 > 0 ⇒ 不得照抄 B1 §3.3 的 UPDATE**，须先决定兜底归属或清脏数据。

### C1 · `POST /api/v1/workforce/agents` —— **不是新增，是拆闸门**

- **实测结论**：该端点**早已存在**（见上「岗位与数字员工目录」节；实现 `app/main.py:1566`，调 `workforce_directory_service.create_employee`）⇒ 本轮**不新增端点**，实为「**拆闸门 + 复用 + 支持"从模板填默认"**」。
- **闸门拆分（三档）**：现 `_require_workforce_directory_admin`（仅 `super_admin`，**实测 9 处调用点**）**须拆为**：

  | 闸门 | 开放给 | 用途 |
  | --- | --- | --- |
  | `_require_directory_reader` | 登录用户（按 **C3** 过滤） | 只读目录 |
  | `_require_employee_creator` | 登录用户（**创建归属自己**） | 员工侧创建 |
  | `_require_workforce_directory_admin`（**保持 `super_admin`**） | 仅 `super_admin` | 建 / 改岗位、改 / **停用他人**员工、读**任意**员工的 `config` |

- **⚠️ 为什么必须拆、不得整体放宽**：直接放宽原闸门会**同时放开**建 / 改岗位、改 / 停用**别人的**员工、读**任意**员工的 `config`（提示词 / 模型 / 日预算 / 审批档）—— 与 `use` 档「能派活、**不能改配置、不能停用**」**直接冲突**。
- 请求体**新增可选** `template_key`（从岗位模板填默认；未知键 `422`）。既有字段与 `409` 口径**不变**。

### C2 · `GET /api/v1/workforce/roles` 与岗位模板列表的关系

- **实测**：`GET /workforce/roles` **早已存在**（同上节）。**V2 = B（允许租户自定义岗位）后模板落库**（表 `workbench_role_templates`）⇒ 两者形态更接近，**边界更须写明**。
- **本轮提案**：`GET /workforce/roles` = **租户内岗位的当前状态**（**既有语义不变**）；**新增** `GET /api/v1/workforce/role-templates` = **平台级预置模板**（`is_builtin` = true，可被租户覆盖）。
- **⚠️ 未定（须评审）**：两者是否应**合并为一个端点** + `source=builtin|custom` 过滤？**本契约不预判。**

### C3 · `GET /api/v1/workforce/agents` —— **过滤与响应体变更**

- **可见性口径（用户已裁 = 两档）**：`可见 = owner_user_id = 我 ∪ 我在 workbench_employee_shares 里 ∪ 我是 super_admin`。
- **⚠️ 原设计的第三档已砍**：`visibility='shared' AND role_key ∈ 我的岗位` —— 经核实仓库**无「用户 → 岗位」映射**（账号侧 `position` 是自由文本，`app/accounts/models.py:54`，账号侧**无 `role_key`**），**缺数据支撑**（`decision-log.md` **D-058③**；与 **B1-V5**「岗位共享档 ⇒ 不做」一致）。
- **⇒ 本条直接修掉「普通员工看到 `403`」。**
- **响应体只增** `owner_user_id`、`visibility`（现响应含 `created_by` 但**无这两个**）；既有字段与分页口径**不变**。
- **前端连带**：「归属无法判定」三分区**可以撤掉**（不再需要容错）。

### C4 · 共享管理 —— **真·新增**（对齐 P2c-6 成员表三端点形态）

| 端点 | 语义 |
| --- | --- |
| `POST /api/v1/workforce/agents/{agent_key}/shares` | 添加共享；请求体 `{"grantee_user_id": string, "permission": "read"\|"use"}`；未知字段 `422` |
| `GET /api/v1/workforce/agents/{agent_key}/shares` | 列出共享者；**须分页**：`limit` 1–200（默认 200）+ `offset` ≥ 0（默认 0）+ `total`；非法值 `422` |
| `DELETE /api/v1/workforce/agents/{agent_key}/shares/{grantee_user_id}` | 撤销共享；成功 `204`；**复删 / 目标不是共享者一律 `204`（幂等 no-op，不重复写审计）** |

- **权限**：**仅归属人**（`owner_user_id` = 我）可增删共享；他人 / 跨租户一律 `404`（**不泄露存在性**）。
- **两档语义**：`read` = 能看配置、看它干过的活，**不能派活**；`use` = 能派活，**不能改配置、不能停用**。
- **审计**：`workforce.agent.share.added` / `workforce.agent.share.removed`；明细**只含** `agent_key` / `grantee_user_id` / `permission`，**不含姓名 / 手机号 / 正文**。
- **不做**：跨租户 / 部门级 / 全租户 / 公开链接共享。
- **派活权两面同口径（2026-09-24 用户裁决；⚠️ 须先于实现落定）**：同租户内，一个数字员工的**派活权在「目录面」与「会话面」口径一致** —— 两处都判「`owner ∪ use`（或 `super_admin`）」，两处的拒绝都是 **`403` 且零落库**、不泄露存在性：
  - **目录面**（直接派活 / 建任务，如 `POST /tasks` 与「存成任务」）：判**调用者本人**是否 `owner ∪ use`（`ensure_can_dispatch`）—— `read` 档与零共享同事一律 `403`。
  - **会话面**（`POST /conversations/{id}/messages` 与 `.../messages:stream` 的**带键执行路径**）：由**会话发起人**享有 `owner ∪ use`（或超管），**且**发言者是该会话的 `write` 成员 —— 即**使用权随会话传递，上限为发起人当前权限**（逐条见「会话协作：分享与多端协同（P2c-6）」节）。
- **⚠️ 由此改变的生产流程（C1 保留的「超管代建员工」）**：`super_admin` 代建后 `owner_user_id` 是**超管本人**（`_resolve_owner`）—— 按上述同口径，**普通用户不再「代建即可用」**：归属人须**显式加 `use` 档共享**，普通用户方可派活（**2026-09-24 用户裁决**；不再存在「代建 ⇒ 隐式可用」的授权）。

### C5 · `autonomy_level` 文案变更

- **落点**：既有契约「岗位与数字员工清单」/「对话式 AI 员工平台」中的 `autonomy_level` 说明处（契约 **767 / 768 行**附近）。
- **实测问题**：现有三档**不表示主动性**（只控制「派活后执行是否要批」）⇒ **界面文案须同步改**。
- **⚠️ V1 = C 口径之后**（主动值班**只做不需审批的动作**），这一档要能表达该语义 ⇒ **措辞需重想**，本轮**不预判**。

### 迁移 045 与租户模板链

- **迁移 045 尚未编写**（加 4 列 + 建 2 表；DDL 见 B1 §3.3 与 B2 施工方案 §1）。
- **⚠️ 租户模板链三处必须同步**：`migrations/tenant_template/0001_tenant_baseline.sql` + `migrations/tenant_template/classification.json` + 跑 `scripts/tenant_schema.py verify --apply`。**漏同步 ⇒ 新租户建出来就是缺列的。**
- **⚠️ 本节全部未实现**。
