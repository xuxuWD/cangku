...
1. **`POST /api/v1/conversations/{conversation_id}/messages`**：P2a-2 起可触发**真实工具执行**，响应 `stub` 由 `true` 变为 `false`；请求体与分页口径不变。执行是**同步、非流式**的（流属 P2b），期间无过程反馈。
2. **`POST /api/v1/conversations`**：`agent_key` **校验收紧**——路由到真实执行前必须校验其在本租户目录中**存在且启用**（复用 `ensure_agent_binding_available`），否则 `422`。**历史会话在数字员工停用后仍必须可读**（沿用 P1 语义，读取不做启用校验）。
3. **`GET /api/v1/audits`**：动作码新增 **`tool.executed`** 与 **`tool.blocked`**；明细走 `ALLOWED_DETAIL_KEYS` 白名单，**不含自由文本**。
...
| ④ 危险命令黑名单 | 命中 §3.2.1 的 A/B/C 任一层 | `403` | 「该命令被禁止执行」 | **命中即拒，不允许审批放行** |
| ⑤ 风险定档 | 定档为需审批（`needs_approval`）**且**尚无授权 | `202` | pending | 先落库审批请求**再**返回；未落库不得进入等待 |
| ⑥ 审批落库 | 落库失败 | `503` | 「审批请求暂时无法登记，请稍后重试」 | 不进入等待（避免 UI orphan） |
| ⑦ 授权位校验 | 未授权 / 计划摘要不一致 / 授权来源不在白名单 | `409` | 「授权已失效，请重新审批」 | 复用迁移 `026`，与 `/runs/{run_id}/approvals/*` 同口径 |
...
- **审计**：`tool.executed`（已执行）与 `tool.blocked`（被拒，含被黑名单/路径/参数拒）；明细只记工具键、风险档、结果状态与**摘要**，**不含自由文本、不含命令原文**。
- **落库**：只落 `摘要 + args_digest + sha256 + 字节数`；**永不落文件正文**。当轮上下文可含原文（送模型），**持久化只落摘要**。
...
| # | 未决点 | 说明 |
| --- | --- | --- |
| Y1 | 是否需要**工具目录只读端点**（如 `GET /api/v1/tools`） | 规格 §3.1 只定义内部声明，未定对外端点；若前端要展示可用工具，需先补规格 |
| Y2 | 工具执行的**运行/审批归属** | 复用会话内的运行（`/api/v1/runs/{run_id}/approvals/*`），还是另起运行？属段二-4 实现细节，但**影响契约**，需先定 |
| Y3 | `tool.executed` 的**明细字段最终名单** | 是否暴露虚拟化路径、是否暴露 `args_digest`；白名单一次定死，避免日后随手加字段 |
