...
> **状态：契约先行 · 草案 · 未实现。** 口径源：`docs/superpowers/specs/2026-09-12-dsh-integration-design.md` §3。
> 落地子段：段二-2（工具目录 + 九步闸门，离线可验收）→ 段二-3（容器执行器 + dsh 适配器）→ 段二-4（对话入口路由 + `agent_key` 校验收紧）。
> 本节**先于代码**存在；若实现与本节冲突，以规格评审结论为准并**回改本节**，不得只改代码。

### 与既有契约的变更点（五处）

1. **`POST /api/v1/conversations/{conversation_id}/messages`**：P2a-2 起可触发**真实工具执行**，响应 `stub` 由 `true` 变为 `false`；请求体与分页口径不变。执行是**同步、非流式**的（流属 P2b），期间无过程反馈，因此**必须有单次执行硬上限**（超时 → `504`，见闸门 ⑧）。
...
3. **`GET /api/v1/audits`**：**必须新增** `AuditAction` 的 **`tool.executed`** 与 **`tool.blocked`**（不是「建议」——未扩动作码时 `?action=tool.executed` 会直接 `422`）；明细走 `ALLOWED_DETAIL_KEYS` 白名单，**不含自由文本**，`reason` 为**受控枚举码**（键与取值见下）。
4. **对话消息端点新增副作用**：触发真实执行时会**自动创建一个「承载任务 + 运行」**（`workbench_run_records.task_id` 保持 `NOT NULL` 不变，零新迁移），审批与授权位复用既有 `/api/v1/runs/{run_id}/approvals/*`。**不新增 `origin` 之类来源字段**：系统无任务列表接口（仅 `POST /api/v1/tasks`、`GET /api/v1/tasks/{task_id}`、`POST /api/v1/tasks/{task_id}/approve`），承载任务无对外列表面，该要求与「零新迁移」互斥，已于 2026-09-12 裁决撤销。
5. **执行幂等**：以 `message_id` 为**执行幂等键**——同一消息重放**不得**产生第二次执行、不得产生第二个承载任务/运行；重复提交返回既有结果。
...
| 步 | 失败条件 | 状态码 | 中文原因 | 备注 |
| --- | --- | --- | --- | --- |
| ① 白名单 | 工具不在当前组装面内 | `422` | 「未知工具」 | 组装时过滤 **且** 执行入口再校验一次 |
...
| ⑤ 风险定档 | 定档为需审批（`needs_approval`）**且**尚无授权 | `202` | pending | 先落库审批请求**再**返回；未落库不得进入等待 |
| ⑥ 审批落库 | 落库失败 | `503` | 「审批请求暂时无法登记，请稍后重试」 | 不进入等待（避免 UI orphan） |
| ⑦ 授权位校验 | 未授权 / **计划摘要或参数摘要不一致** | `409` | 「授权已失效，请重新审批」 | 复用迁移 `026`；**授权来源白名单校验发生在审批写入阶段**，不在本步 |
| ⑧ 容器执行 | 超时 / 容器异常 / 非 2xx | `504`（超时）或 `502` | 「执行超时，已终止」/「执行失败」 | **超时语义 = 拒绝并终止容器**（fail-closed） |
| ⑨ 结果落库 | — | — | — | 只落**摘要**（见下） |

> **审批通过后的推进（2026-09-12 修订补）**：审批决议端点通过后，**在同一请求内从 ① 重跑全套闸门**（含 ②③④）再进入 ⑧。**不存在**任何跳过 ②③④ 的「已校验」续跑路径；**不得**以「重新发消息」作为推进方式（那会产生新运行与新授权位，⑦ 将永不可达）。
...
- **`reason` 为受控枚举码**（`path_denied` / `blacklisted` / `param_invalid` / `not_authorized` / `timeout` / `runtime_error`），**不得写入自由文本**——否则命令串、路径、异常 message 会随它落进审计。
...
- **不新建表**（X5）：工具调用进既有 append-only 审计；不建「工具调用记录表」。
...
| Y2 | 工具执行的**运行/审批归属** | **自动创建「承载任务 + Run」**并复用既有 `/api/v1/runs/{run_id}/approvals/*`（含授权位 `026`），**零新迁移**；**不新增 `origin` 字段**（2026-09-12 裁决撤销）；幂等键由 `message_id` 派生 |
...
> 上述三项与本节其余内容一样**尚未实现**；实现时若与本表冲突，同样以规格评审结论为准并回改本节。
