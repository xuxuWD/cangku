...
> 通知能力的**当前范围**：① 死信运维通知渠道（脱敏 webhook，`app/notifications.py`，未配置地址则不发送）；② 面向员工的**站内通知收件箱**（`app/inbox.py` + `migrations/019_inbox_items.sql`，覆盖任务审批、计划与编排提案审核、内容发布转人工接管、运行失败与被取消、运行内审批被驳回、账号注册通过，见 `docs/api-contract.md`「站内通知（收件箱）」）。**仍未实现**：服务端推送；两端提醒均为**客户端轮询**。运行记录会在终态落盘并带受控的 `finish_reason`（迁移 `020`）。
