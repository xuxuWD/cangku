...
function routeFromLocation() {
  ...
  const known = view === 'history' || view === 'knowledge' || view === 'dynamics' || view === 'inbox' || view === 'audit'
...
export default function App() {
  ...
  const navigate = (view: AppView, taskId?: string, runId?: string) => { const query = view === 'history' ? '?view=history' : view === 'knowledge' ? '?view=knowledge' : view === 'dynamics' ? '?view=dynamics' : view === 'inbox' ? '?view=inbox' : view === 'audit' ? '?view=audit' : view === 'run' ? `?view=run&run=${encodeURIComponent(runId ?? '')}` : taskId ? `?task=${encodeURIComponent(taskId)}` : ''; window.history.pushState({}, '', query || window.location.pathname); window.dispatchEvent(new PopStateEvent('popstate')) }
