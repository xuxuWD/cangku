...
function routeFromLocation() {
  ...
  return { view: (view === 'history' || view === 'knowledge' || view === 'dynamics' ? view : 'workbench') as AppView, taskId: params.get('task') || undefined }
}
...
export default function App() {
  ...
  const navigate = (view: AppView, taskId?: string) => { const query = view === 'history' ? '?view=history' : view === 'knowledge' ? '?view=knowledge' : view === 'dynamics' ? '?view=dynamics' : taskId ? `?task=${encodeURIComponent(taskId)}` : ''; window.history.pushState({}, '', query || window.location.pathname); window.dispatchEvent(new PopStateEvent('popstate')) }
