...
function routeFromLocation() {
  ...
  return { view: (view === 'history' || view === 'knowledge' || view === 'dynamics' || view === 'inbox' ? view : 'workbench') as AppView, taskId: params.get('task') || undefined }
}
...
export default function App() {
  ...
  const navigate = (view: AppView, taskId?: string) => { const query = view === 'history' ? '?view=history' : view === 'knowledge' ? '?view=knowledge' : view === 'dynamics' ? '?view=dynamics' : view === 'inbox' ? '?view=inbox' : taskId ? `?task=${encodeURIComponent(taskId)}` : ''; window.history.pushState({}, '', query || window.location.pathname); window.dispatchEvent(new PopStateEvent('popstate')) }
  ...
  if (route.view === 'inbox') return <InboxPage onOpenTask={(taskId) => navigate('workbench', taskId)} onNavigate={(view) => navigate(view)} />
