...
export function CrmProgressPage({ onNavigate }: { onNavigate?: (view: AppView) => void } = {}) {
  ...
  return <main className="main-content content-history crm">
    <div className="page-head">
      <div>
        <h1 className="page-title">进度概览</h1>
    ...
    <div className="toolbar">
      <div className="segment" role="tablist" aria-label="统计范围">
        <button role="tab" aria-selected={scope === 'me'} className={scope === 'me' ? 'active' : ''} onClick={() => setScope('me')}>我的</button>
        <button role="tab" aria-selected={scope === 'all'} className={scope === 'all' ? 'active' : ''} onClick={() => setScope('all')}>全量</button>
      </div>
      <span className="role-note">scope=all 需要部门负责人 / CEO / 超级管理员岗位，服务端判定（否则 403）。</span>
    </div>
    ...
    {!loading && error && <CrmNotice
      tone="error"
      title={scope === 'all' && error.status === 403 ? '当前账号无权查看全量指标' : '进度指标加载失败'}
      action={error.retryable ? <button className="text-action" type="button" onClick={() => void load()}>重新尝试</button> : undefined}
    >
      <p>{error.message}</p>
      {scope === 'all' && error.status === 403 && <p>全量口径（scope=all）仅对部门负责人 / CEO / 超级管理员开放；可切换回「我的」。</p>}
    </CrmNotice>}

    {!loading && !error && metrics && <>
      <section className="history-panel crm__panel" aria-label="管线与赢率">
        <div className="panel-header"><h2>管线与赢率</h2><span>{scope === 'all' ? '全量' : '我的'}</span></div>
      ...
      <section className="history-panel crm__panel" aria-label="健康分档分布">
        <div className="panel-header"><h2>健康分档分布</h2><span>客户计数</span></div>
        <div className="panel-body"><div className="stat-grid">
      ...
      <section className="history-panel crm__panel" aria-label="续约与回款">
        <div className="panel-header"><h2>续约窗口与回款</h2><span>仅 signed 合同</span></div>
        <div className="panel-body"><div className="stat-grid">
          ...
          <MetricTile label="已到期未结清" value={`${formatCount(metrics.overdue_contract_count)} 份`} hint="signed 且已过 ends_on 且回款未满。" />
        </div></div>
      </section>

      <section className="history-panel crm__panel" aria-label="目标达成度">
        <div className="panel-header"><h2>目标达成度（当月）</h2><span>{targetNoTarget ? '无目标' : '有目标'}</span></div>
        <div className="panel-body"><div className="stat-grid">
